...
def GetDependency(node):
    ...
    #         key  = 'asset:%s'%vset[0]
    #         vset = vset[1]
    #         if not res[key].get(vset):
    #             res[key][var.ORDER].append(vset)
    #         res[key][vset] = val
    #     else:
    #         # find variantSet in shot
    #         if res.get(var.T.SHOT):
    #             if not res[var.T.SHOT].get(vset):
...
def GetCustomLayerData(node, pub):
    ...
    for k in [var.T.BRANCH, var.T.ASSET]:
        ...
        # set task
        dtask = None
        if depdata[k].get(var.T.TASK):
            dtask = depdata[k][var.T.TASK]

        # set rigFile
        if resdata['sequenced'] and not data.get(var.T.CUS_RIGFILE):
        ...
        if resdata['lyrtype'] == var.LYRGROOM:
            ...
            if dtask == var.T.RIG:
                ...
                if isSeq:
                    data[var.T.CUS_GROOMFILE] = GetGroomFile(rigfile)

        elif resdata['lyrtype'] == var.LYRFEATHER:
            if not data.get(var.T.CUS_FEATHERFILE):
...
def ResolveDependency(kwargs):
    ...
    if kwargs.get('script_multiparm_index'):
    ...
    if required:
        ...
        # Instance
            ...
            protopath = node.parm('inst_scatterprototypespath').evalAsString()
            if 'importPrims' in protopath:
                node.parm('inst_useprim').set(1)
        ...
        # find import nodes
        if targetNode:
            # if type == var.LYRGROOM:
            #     ntl.GetGroomDependency(targetNode, 'dxusdSOP_import', impnodes,
            #                            firstMatch=False)
            # else:
...
def ResolveLayer(kwargs):
    ...
    # --------------------------------------------------------------------------
        ...
        info['dprim'] = 'Groom'
        info['lyrname'] = soppath if soppath else var.UNKNOWN
        info['nslyr'] = obj.path().split('/')[-1] if obj else var.UNKNOWN
