...
def spool(xlsFileName, isShotNameBurn, isEffectBurn, uploadSeqMov, plateType, applyEditOrder, skipTactic):
    ...
    while row < sheet.nrows:
        ...
        isModify = False
        edirOrderIndex = rowData[Column2.EDIT_ORDER.value]
        ...
        if row + 1 != sheet.nrows:
            while sheet.row_values(row + 1)[Column2.SHOT_NAME.value] == shotName:
                otherRowData = sheet.row_values(row + 1)
                shotDetailDict[index] = {Column2.CLIP_NAME.name.lower(): otherRowData[Column2.CLIP_NAME.value],
                                         Column2.TYPE.name.lower(): otherRowData[Column2.TYPE.value],
                                         Column2.ISSUE.name.lower(): otherRowData[Column2.ISSUE.value]}
                row += 1
        ...
        command += ['-t', '%s-%s' % (int(rowData[Column2.MOV_CUT_IN.value]), int(rowData[Column2.MOV_CUT_IN.value] + rowData[Column2.MOV_CUT_DURATION.value]) - 1)]
        command += ['-in709', '-outsrgb']

        padding = len(str(int(rowData[Column2.MOV_CUT_IN.value])))
        if padding < len(str(int(rowData[Column2.MOV_CUT_IN.value]) + int(rowData[Column2.MOV_CUT_DURATION.value]) - 1)):
            padding = len(str(int(rowData[Column2.MOV_CUT_IN.value]) + int(rowData[Column2.MOV_CUT_DURATION.value]) - 1))

        dstJpgDir = os.path.join(movDirectory, '_shot_burn_in_', '%s_%s' % (shotName, clipPlateType))
        dstJpgRule = os.path.join(dstJpgDir, '%s_%s.' % (shotName, clipPlateType) + '%0' + str(padding) + 'd.jpg')
        ...
        soundCommand += ['-t', '%s-%s' % (int(rowData[Column2.MOV_CUT_IN.value]),
                                     int(rowData[Column2.MOV_CUT_IN.value] + rowData[Column2.MOV_CUT_DURATION.value]) - 1)]

        wavFilePath = os.path.join(movDirectory, '_shot_burn_in_', '%s_%s.wav' % (shotName, clipPlateType))
        ...
        if not isShotNameBurn:
            ...
            nukeCommand += ['--shotname', rowData[Column2.SHOT_NAME.value]]
        ...
        if not isEffectBurn:
            ...
            effectText = rowData[Column2.ISSUE.value]
            ...
            if shotDetailDict:
                for key in sorted(shotDetailDict.keys()):
                    ...
                    for text in shotDetailDict[key][Column2.ISSUE.name.lower()].split('\n'):
                        if "SpeedRamp" in text or "Retime" in text or "Scale" in text or "Rotation" in text:
                            nukeEffectText += '%s,-,%s,-,%s--' % (clipName, clipPlateType, text.replace(' ', ','))
                            cleanupEffectText += '%s - %s - %s\n' % (clipName, clipPlateType, text)
        ...
        # Third Jpg To Mov
        burnInMovFileName = os.path.join(movDirectory, '_shot_burn_in_', '%7d_%s_%s.mov' % (edirOrderIndex, shotName, clipPlateType))
        movCommand = ['/backstage/dcc/DCC', 'rez-env', 'ffmpeg_toolkit', '--']
        movCommand += ['ffmpeg_converter', '-i', os.path.join(dstJpgDir, burnInDirName)]
        movCommand += ['-r', '%s' % '23.98']# rowData[Column2.MOV_CUT_FPS.value]]
        ...
        shotThumbnailCommand += ['--movFile', os.path.join(dstJpgDir, burnInDirName, '%s.%05d.jpg' % (shotName, rowData[Column2.MOV_CUT_IN.value]))]
        ...
        shot_data = {
            ...
            'frame_out': int(rowData[Column2.FRAME_OUT.value]),
            'edit_order': int(rowData[Column2.EDIT_ORDER.value]),
        ...
        if queryData:
            ...
            if curFrameIn and curFrameIn != rowData[Column2.FRAME_IN.value]:
                data['frame_in'] = int(rowData[Column2.FRAME_IN.value])
                offsetValue = curFrameIn - rowData[Column2.FRAME_IN.value]
                if offsetValue > 0:
                    editPos = "top"
                    editCompare = "add"
                    editDur = offsetValue
                    ...
                    editCompare = "delete"
                    editDur = -offsetValue
            if curFrameOut and curFrameOut != rowData[Column2.FRAME_OUT.value]:
                data['frame_out'] = int(rowData[Column2.FRAME_OUT.value])
                offsetValue = curFrameOut - rowData[Column2.FRAME_OUT.value]
                if offsetValue > 0:
                    editPos = "end"
                    editCompare = "delete"
                    editDur = offsetValue
                    ...
                    editCompare = "add"
                    editDur = -offsetValue
            if editOrder != rowData[Column2.EDIT_ORDER.value] and applyEditOrder:
                data['edit_order'] = int(rowData[Column2.EDIT_ORDER.value])
            ...
            if data != {}:
                ...
                if editPos and editCompare and editDur:
                    note = FORMAT.EDIT_CHANGED_MSG.format(EDITDATE=editDate,
                                                          ...
                                                          NEW_FRAME_IN_OUT='%d-%d' % (int(rowData[Column2.FRAME_IN.value]), int(rowData[Column2.FRAME_OUT.value])),
                                                          NEW_DURATION=int(rowData[Column2.FRAME_OUT.value]) - int(rowData[Column2.FRAME_IN.value]) + 1)
                    print shotQueryData
                    print note
    ...
    if uploadSeqMov:
        for seqName in shotMovFileDict.keys():
            ...
            for shotMov in sorted(shotMovFileDict[seqName]):
                dirName = os.path.dirname(shotMov)
                basename = os.path.basename(shotMov)
                splitBaseName = basename.split('_')
                shotMovName = '_'.join(splitBaseName[1:])
                file = glob.glob('%s/*%s' % (dirName, shotMovName))
                print '%s/*%s' % (dirName, shotMovName), file
                seqMovCommand.append(file[0])
...
if __name__ == '__main__':
    ...
    argparser.add_argument('-st', '--skipTactic', dest='skipTactic', type=str, help='skip tactic process')
    ...
    skipTactic = False
    if args.skipTactic == "True":
        skipTactic = True

    spool(args.file, shotNameBurn, effectBurn, velozSeqMov, args.plateType, editOrder, skipTactic)
