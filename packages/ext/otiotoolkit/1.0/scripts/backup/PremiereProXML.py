    ...
    def doIt(self):
        ...
        while cutIndex < len(editCutInFrameList):
            ...
            for clipIndex, clip in enumerate(clipData):
                ...
                # if clip.schema_name() == "Clip" and clip.media_reference.is_missing_reference:  # source file missing
                #     editOrder -= 1
                #     break

                # Check Time Base
                if clip.schema_name() != "Stack" and clip.media_reference.available_range is None:
                    continue

                orgFpsBase = clip.available_range().start_time.rate
                startTime = clip.visible_range().start_time
                durationTime = clip.visible_range().duration

                # Disable NTSC
                if clip.schema_name() != "Stack" and clip.media_reference.metadata.get('fcp_xml') and clip.media_reference.metadata['fcp_xml']['rate'].get('ntsc') == "TRUE":
                ...
                if hasattr(clip, 'effects'):
                    ...
                    if retimeSpeed != '0' and availableRetimeSpeed(retimeSpeed):  # has Retime
                        ...
                        try:
                            ...
                            curReelName = clip.media_reference.name
                        ...
                        if preOrderReelName == curReelName and isReverse == isPreOrderReverse:
                            ...
                            if preOrderHasEffect:
                                ...
                                issue += FORMAT.SPEEDRAMPTC.format(RETIME=preOrderRetimeSpeed,
                                                                   TC_IN=preOrderRetimeRange.start_time.to_timecode(),
                                                                   TC_OUT=preOrderRetimeRange.end_time_exclusive().to_timecode())

                            # speedRampText += FORMAT.SPEEDRAMP.format()
                            retimeRange = otio.opentime.TimeRange(startTime, durationTime)
                            issue += FORMAT.SPEEDRAMPTC.format(RETIME=retimeSpeed,
                                                               TC_IN=retimeRange.start_time.to_timecode(),
                                                               TC_OUT=(retimeRange.end_time_exclusive() + otio.opentime.RationalTime(1 * (iRetimeSpeed * 0.01), fpsBase)).to_timecode())
                            ...
                            issue += FORMAT.SPEEDRAMPTC.format(RETIME=retimeSpeed,
                                                               TC_IN=retimeRange.start_time.to_timecode(),
                                                               TC_OUT=(retimeRange.end_time_exclusive() + otio.opentime.RationalTime(1 * (int(float(postOrderRetimeSpeed)) * 0.01), fpsBase)).to_timecode())
                            ...
                            if postOrderHasEffect:
                                ...
                                issue += FORMAT.SPEEDRAMPTC.format(RETIME=postOrderRetimeSpeed,
                                                                   TC_IN=postOrderRetimeRange.start_time.to_timecode(),
                                                                   TC_OUT=(postOrderRetimeRange.end_time_exclusive()).to_timecode())
                ...
                self.excelSheet.write(row, Column2_org.EDIT_ORDER.value, editOrder)

                self.excelSheet.write(row, Column2_org.TC_IN.value, editTCRange.start_time.to_timecode())
                if (retimeSpeed != "0" and availableRetimeSpeed(retimeSpeed)) or (postOrderRetimeSpeed == '0' and availableRetimeSpeed(postOrderRetimeSpeed)):
                    endTC = editTCRange.end_time_exclusive()
                    # print editTCRange.duration, endTC
                    self.excelSheet.write(row, Column2_org.TC_OUT.value, editTCRange.end_time_exclusive().to_timecode())
                else:
                    endTC = editTCRange.end_time_inclusive()
                    # print editTCRange.duration
                    self.excelSheet.write(row, Column2_org.TC_OUT.value, editTCRange.end_time_inclusive().to_timecode())

                try:
                    clipName = os.path.basename(clip.media_reference.target_url).split('.')[0]
                except:
                    try:
                        clipName = clip.media_reference.name.split('.')[0]
                    except:
                        clipName = ''
                ...
                if findItem:
                    if startTC != findItem[Column2_org.TC_IN.name.lower()]:
                        Msg.warning("startTC :", startTC, "alreadyTC_IN :", findItem[Column2_org.TC_IN.name.lower()])
                        originalTC_IN = otio.opentime.RationalTime.from_timecode(findItem[Column2_org.TC_IN.name.lower()],
                                                                                 fpsBase)
                        offset = originalTC_IN - editTCRange.start_time
                        frameIn = findItem[Column2_org.FRAME_IN.name.lower()]
                        frameIn -= offset.value
                        if offset.value <= 0:  # nag value
                            issue += "top delete %d\n" % -offset.value
                        else:
                            issue += "top add %d\n" % offset.value
                    if endTC.to_timecode() != findItem[Column2_org.TC_OUT.name.lower()]:
                        Msg.warning("endTC :", endTC.to_timecode(), "alreadyTC_OUT :",
                                    findItem[Column2_org.TC_OUT.name.lower()])
                        originalTC_OUT = otio.opentime.RationalTime.from_timecode(findItem[Column2_org.TC_OUT.name.lower()],
                                                                                  fpsBase)
                ...
                if findItem and findItem.has_key(Column2_org.SHOT_NAME.name.lower()):
                    self.excelSheet.write(row, Column2_org.SHOT_NAME.value, findItem[Column2_org.SHOT_NAME.name.lower()])
                elif self.cleanupTimeline[editCutInFrameList[cutIndex]].has_key('shotName'):
                    self.excelSheet.write(row, Column2_org.SHOT_NAME.value,
                                          self.cleanupTimeline[editCutInFrameList[cutIndex]]['shotName'])

                if findItem and findItem.has_key(Column2_org.TYPE.name.lower()):
                    self.excelSheet.write(row, Column2_org.TYPE.value, findItem[Column2_org.TYPE.name.lower()])
                else:
                    self.excelSheet.write(row, Column2_org.TYPE.value, plateType)

                if findItem and findItem.has_key(Column2_org.VERSION.name.lower()):
                    self.excelSheet.write(row, Column2_org.VERSION.value, findItem[Column2_org.VERSION.name.lower()])

                self.excelSheet.write(row, Column2_org.FRAME_IN.value, frameIn)
                self.excelSheet.write(row, Column2_org.FRAME_OUT.value,
                                      frameIn + orgDurationTime.to_frames() + addMovCutDuration.to_frames() - 1)
                self.excelSheet.write(row, Column2_org.SHOT_DURATION.value,
                                      orgDurationTime.to_frames() + addMovCutDuration.to_frames())
                self.excelSheet.write(row, Column2_org.ORIGINAL_ROOT_FOLDER.value, "/stuff/%s/scan" % self.showName)

                if findItem and findItem.has_key(Column2_org.VERSION.name.lower()):
                    self.excelSheet.write(row, Column2_org.ORIGINAL_ROOT_PATH.value,
                                          findItem[Column2_org.ORIGINAL_ROOT_PATH.name.lower()])

                if "prv" not in clipName:
                    self.excelSheet.write(row, Column2_org.CLIP_NAME.value, clipName)

                self.excelSheet.write(row, Column2_org.SCAN_DURATION.value, "%s" % editTCRange.duration.to_frames())

                in_offset = otio.opentime.RationalTime(0, 24)
                out_offset = otio.opentime.RationalTime(0, 24)
                if self.cleanupTimeline[editCutInFrameList[cutIndex]].has_key('in_offset') or self.cleanupTimeline[
                    editCutInFrameList[cutIndex]].has_key('out_offset'):
                ...
                self.excelSheet.write(row, Column2_org.ISSUE.value, issue.strip())
                # sheet.write(row, Column2_org.RESOLUTION.value, "1920x1080")

                self.excelSheet.write(row, Column2_org.XML_NAME.value, self.editFilePath)
                self.excelSheet.write(row, Column2_org.MOV_CUT_IN.value,
                                      self.globalStartFrame + editCutInFrameList[cutIndex] - in_offset.value)
                try:
                    duration = editCutInFrameList[cutIndex + 1] - editCutInFrameList[
                        cutIndex] + addMovCutDuration.to_frames()
                    self.excelSheet.write(row, Column2_org.MOV_CUT_DURATION.value,
                                          duration + in_offset.value + out_offset.value)
                except Exception as e:
                ...
                metadataList.append({'START_TC': editTCRange.start_time.to_timecode(),
                                       'END_TC': endTC.to_timecode(),
                                       'CLIPNAME': clipName})
                movClip = otio.schema.Clip(name=clipName,
                                           source_range=otio.opentime.TimeRange(otio.opentime.RationalTime(
                                               self.globalStartFrame + editCutInFrameList[cutIndex], fpsBase),
                                                                                otio.opentime.RationalTime(duration,
                                                                                                           fpsBase)),
                                           media_reference=mediaReference)
                if clipIndex == 0:
                    movTrack.append(movClip)

                self.excelSheet.write(row, Column2_org.MOV_CUT_FPS.value, "%0.2f" % orgFpsBase)
            ...
            if len(movTrack) > 0:
                movTrack[-1].metadata.update({'DEXTER': metadataList})
