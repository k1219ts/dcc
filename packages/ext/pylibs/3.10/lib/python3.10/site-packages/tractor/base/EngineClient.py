...
class EngineClient(object):
    ...
    def __init__(self, hostname=None, port=None, user=None, password=None, sessionFilename=None, debug=False, logger=None):
    ...
    def dprint(self, msg):
        ...
        if self.debug:
            msg = "[%s:%s] %s" % (self.hostname, self.port, msg)
            if self.logger:
    ...
    def dprintUrl(self, url):
        ...
        if self.debug:
            msg = "[%s:%s] http://%s:%s/Tractor/%s" % (self.hostname, self.port, self.hostname, self.port, url)
            if self.logger:
                self.logger.debug(msg)
            else:
                trutil.log(msg)
    ...
    def constructURL(self, queryType, keyValuePairs):
        """Build a URL."""
        parts = []
        for key, value in iter(keyValuePairs.items()):
    ...
    def getTaskLog(self, jid, tid, owner=None):
        ...
        for logURI in logURIs:
            ...
            # also convert from network bytes to python3 strings
    ...
    def fetchJobDetails(self, jid, graph=True, notes=True):
        """Return the job information in JSON format."""
        attrs = {"q": "jobdetails", "jid": jid, "graph": int(graph), "notes": int(notes), "flat": 1}
        jobDetails = self._transaction(self.MONITOR, attrs)
        return jobDetails
    ...
    def nextCmd(self, probe=False, **argv):
        ...
        attrs.update(argv)
        try:
            result = self._transaction(self.TASK, attrs)
        except TransactionError as err:
            msg = str(err)
            if "error 404:" in msg or "job queue is empty" in msg or "no dispatchable tasks" in msg or \
                   "no remaining queueable commands" in msg:
                return []
            else:
                raise
        return result
    ...
    def getConfig(self, filename):
        ...
        attrs = {"q": "get", "file": filename}
        return self._transaction(self.CONFIG, attrs)
