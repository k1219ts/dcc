...
class UnknownTerminalColor(Exception):
    pass
...
class TerminalColor:
    ...
    def __init__(self, fg, bg=None):
        ...
        if fg not in self.colorcodes['fg']:
            raise UnknownTerminalColor("'%s' is an unknown foreground color, must " \
                  "be one of %s" % (fg, self.colorcodes['fg'].keys()))
        self.foreground = fg
        if bg and bg not in self.colorcodes['bg']:
            raise UnknownTerminalColor("'%s' is an unknown background color, must " \
