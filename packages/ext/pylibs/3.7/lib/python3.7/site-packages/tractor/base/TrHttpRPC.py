...
class TrHttpRPC (object):
    ...
    def Transaction (self, tractorverb, formdata, parseCtxName=None,
        ...
        try:
            ...
            if hsock:
                ...
                hsock.sendall( req.encode('utf-8') )   ## -- send the request! -- ##
            ...
            errcode = e.errno
            outdata = {"msg": "http transaction: " + e.strerror}
    ...
    def httpConnect (self):
        ...
        try:
            ...
            if errcode in (errno.ECONNREFUSED, errno.WSAECONNREFUSED):
                outdata += "connection refused"
            elif errcode in (errno.ECONNRESET, errno.WSAECONNRESET):
                outdata += "connection dropped"
            elif errcode in (errno.ETIMEDOUT, errno.WSAETIMEDOUT):
                outdata += "connect attempt timed-out (routing? firewall?)"
            elif errcode in (errno.EHOSTUNREACH, errno.ENETUNREACH, errno.ENETDOWN):
    ...
    def collectHttpReply (self, hsock, parseCtxName):
        ...
        out = b""  # build up the reply text (network bytes)
        err = 0
        ...
        while 1:
            try:
                ...
                mustTimeWait = True
                out = "error waiting for http reply "+errnm+" "+str(e)
                try:
                    err = e.errno
                except:
                    err = 999
    ...
    def PasswordRequired(self):
        ...
        try:
            ...
            err, data = self.Transaction("config?file=trSiteFunctions.py",
                                            None, None)
            if data and not err:
                exec(data, globals())  # instantiate function defs (use imp someday)
