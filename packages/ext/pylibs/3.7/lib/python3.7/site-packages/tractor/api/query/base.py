...
def instances(search="", columns=[], sortby=[], limit=None, archive=False):
    """Retrieve a list of instances."""
    return _tractorSelect("instance", where=search, columns=columns, sortby=sortby, limit=limit, archive=archive)
...
def delay(search="", sortby=[], limit=None, aftertime=None):
    ...
    if aftertime is None:
        ...
        aftertime = int(aftertime.strftime("%s"))
    elif isinstance(aftertime, str):
        try:
            aftertime = int(datetime.datetime.strptime(aftertime, "%Y-%m-%d %H:%M:%S").strftime("%s"))
