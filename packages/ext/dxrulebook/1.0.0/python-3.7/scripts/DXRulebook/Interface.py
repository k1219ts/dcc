...
def CustomChangingFlags(res, rb):
    ...
    if 'task' in res and res['task'] == 'ZN':
...
class _Products(object):
    ...
    def __getattr__(self, k):
        if k in self.__dict__:
            return self.__dict__[k]
        elif self._dcc and k in self[self._dcc].__dict__:
    ...
    def SetFlags(rb, kwargs):
        ...
        for k in kwargs.keys():
            if k in rb.flags:
                if isinstance(kwargs[k], str) and \
    ...
    def TreatABName(res, isBranch):
        if 'abname' in res:
            ...
            res['__isbranch__'] = 'branch' in res
...
class Tags:
    ...
    def __getattr__(self, k):
        if k in self.__dict__:
            return self.__dict__[k]
        elif self._dcc and k in self.__dict__[self._dcc]._tags:
...
class Parsed(_BaseFlagDict):
    ...
    def ABName(self):
        if 'branch' in self and self.IsBranch():
            return self.branch
        elif 'asset' in self:
            return self.asset
        else:
...
class Coder:
    ...
    def Encode(self, **kwargs):
        ...
        if 'pub' in rb.flags and not 'pub' in kwargs:
...
class Flags(_BaseFlagDict):
    ...
    def HasBranch(self):
        return 'branch' in self and self.branch

    def HasAsset(self):
        return 'asset' in self and self.asset
