...
class Coder(object):
    ...
    def __init__(self, family='Global', env=os.environ, parent=None):
        ...
        try:
            self._default_keys = list(self.__dict__.keys()) + list(Coder.__dict__.keys())
        except:
            self._default_keys = list(self.__dict__.keys()) + list(Coder.__dict__.keys())
        return
    ...
    def _inherit(self, parent):
        ...
        '''
        for key, flag in list(parent._flag.items()):
            self._flag[key] = flag
            flag._users.append(self)

        for key, tag in list(parent._tag.items()):
            self._tag[key] = tag
    ...
    def products(self):
        ...
        Returns:
            (list) product names
        '''
        return list(self._product.keys())
    ...
    def myProducts(self):
        ...
        '''
        return list(self._myProduct.keys())
    ...
    def _decode_expression(self, product_name='path', exps=None, s='^', e='$'):
        ...
        for exp in exps:
            ...
            for key, flag in list(self._flag.items()):
    ...
    def update_rule(self, rule_data, recursive=False):
        ...
        self._myFlags = list(rule_data.get('__flag__', {}).keys())
        for key, attr in list(rule_data.get('__flag__', {}).items()):
            if key not in self._flag_raw_data:
                self._flag_raw_data[key] = {}
            self._flag_raw_data[key].update(attr)

        self._myTags = list(rule_data.get('__tag__', {}).keys())
        for key, val in list(rule_data.get('__tag__', {}).items()):
        ...
        if recursive:
            ...
            for key, val in list(children_data.items()):
    ...
    def _initialize_tags(self):
        for key, attr in list(self._tag_raw_data.items()):
            tag = Coder.Tag()
        ...
        roopCnt = 5
        keys = list()
        for k, tag in list(self._tag.items()):
            if '@' in tag.value:
                keys.append(k)
                if '|' in tag.value:
                    tag.rule = tag.value
    ...
    def _initialize_flags(self):
        ...
        for key, attr in list(self._flag_raw_data.items()):
            flag = Coder.Flag()
            # substitute tags
    ...
    def load_yaml(yml_path):
        ...
        with open(yml_path, 'r') as yml_file:
            data = yaml.full_load(yml_file.read())
