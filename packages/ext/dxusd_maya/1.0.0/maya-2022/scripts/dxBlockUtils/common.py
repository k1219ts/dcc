...
class ImportGeomPostProcess:
    ...
    def createReferenceNode(self, prim, filePath, primPath="", excludePrimPaths="", variants=[]):
        ...
        mutl.SetDefaultTransform(refNode)
    ...
    def references_doIt(self, prim):
        ...
        try:
            ...
            #Feather src layer imported
            if stack[-1]:
                if '/Groom' in str(stack[-1].path):
                    sceneGraphPath = prim.GetPath().pathString
                    mayaPath = sceneGraphPath.replace('/', '|')
                    splitMayaPath = mayaPath.split('|')
                    dagPath = self.rootNode + '|' + '|'.join(splitMayaPath[2:])
                    ...
                    if cmds.objExists(dagPath):
                        node = dagPath
                        srcfile = stack[-1].layer.identifier
                        primpath = str(stack[-1].path)
                        mutl.SetFeatherAttr(node, srcfile, primpath)
                        mutl.SetDefaultTransform(node)

                        deleteList = [cmds.ls('%s|Feather' % dagPath, type='transform')[0],cmds.ls('%s|Guides' % dagPath,type='transform')[0]]
                        if deleteList:
    ...
    def references_getInfo(self, spec):
        ...
        # custom data
        if spec.customData.get('excludePrimPaths'):
            data['excludePrimPaths'] = spec.customData.get('excludePrimPaths')
        if spec.customData.get('primPath'):
    ...
    def specializes_getInfo(self, spec):
        ...
        if not self.specializes_DATA.get(spec.path):
