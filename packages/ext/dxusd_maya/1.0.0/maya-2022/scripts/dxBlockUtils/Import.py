...
class UsdImport:
    ...
    def importGeom(self, filename, meta=None):
        ...
        nodes = cmds.usdImport(f=filename, shd=["none", "default"])
    ...
    def getRigGeomFilename(self):
        ...
        if '/sim/' in self.inputfile:
            if rootLayer.customLayerData.get('rigFile'):
...
def UsdFeatherImportDialog():
    fn = cmds.fileDialog2(fm=1, ff='USD (*.usd)')
    if not fn:
        return
    impnodes = cmds.usdImport(f=fn[0])
    UsdFeatherAttr(impnodes,fn[0] )


def UsdFeatherAttr(nodes, srcfile):
    import DXUSD_MAYA.MUtils as mutl

    deleteList = []
    for grp in nodes:
        for nullnode in cmds.ls('%s|*|Feather' % grp, type='transform') :
            deleteList.append(nullnode)
        for nullnode in cmds.ls('%s|*|Guides' % grp, type='transform'):
            deleteList.append(nullnode)
    if deleteList:
        cmds.delete(deleteList)

    for node in cmds.ls('%s|*' % nodes[0], type='transform'):
        primpath = '/Groom/%s' % node
        mutl.SetFeatherAttr(node, srcfile, primpath)
        mutl.SetDefaultTransform(node)
