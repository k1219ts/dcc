...
class UsdMerge:
    ...
    def doIt(self):
        ...
        if ret['task'] == 'sim': # if sim
            ...
            if customLayerData.get('inputCache'):
                ...
                if primType == 'Mesh':
                    ...
                    if cmds.listConnections(shape, type=_pxrPBD, s=True):
                        ...
                        if '_sim_' in deformNode:
                            ...
                            # cmds.connectAttr('%s.outputGeometry[0]' % deformNode, '%s.inMesh' % shape)
                            
                            node = cmds.deformer(shape, type=_pxrPBD, name='%s_sim_%s' % (shape.split('|')[-1], _pxrPBD))[0]
                            cmds.setAttr('%s.primPath' % node, primPath, type='string')
                            cmds.connectAttr('%s.outUsdStage' % stageNode, '%s.inUsdStage' % node)
                            cmds.connectAttr('time1.outTime', '%s.time' % node)
                            #:267 disconnectionError(maya 2022)
                            aniNode = node.replace('_sim_','_ani_')
                            oriShape = cmds.listConnections('%s.input[0].inputGeometry' % aniNode,sh = 1)[0]
                            cmds.disconnectAttr('%s.outputGeometry[0]' % aniNode, '%s.input[0].inputGeometry' % node)
                            cmds.connectAttr('%s.worldMesh[0]' % oriShape, '%s.input[0].inputGeometry' % node )
                            ##
                            if usdAttrs:
                                cmn.SetUsdAttributes(shape, usdAttrs)
                    else:
