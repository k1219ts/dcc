...
##########################################
__author__  = '''
    kwantae.kim in Dexter CGsupervisor
'''
__date__ = '2022.06.30'
##########################################
...
class AssetNameItem(QtWidgets.QTreeWidgetItem):
    ...
    def getAssetType(self, assetType):
        if self.hasTypeDict.get(assetType):
    ...
    def updateLastVersion(self):
        if isinstance(self.versionEdit, QtWidgets.QLineEdit):
            if self.kwargs['task'] == Define.RIG_TYPE or self.kwargs['task'] == Define.GROOM_TYPE:
                ...
                if self.overWrite:
                    self.versionEdit.setText(chkVer(utls.GetLastVersion(self.assetDir)))
                else:
                    self.versionEdit.setText(chkVer(utls.GetNextVersion(self.assetDir)))
