    ...
    for i in infos:
        ...
        if os.path.splitext(path)[-1] == ".mov":
            try:
               ...
               continue
            if not m.userMetaData.get("mayaFilePath"):
                continue
...
def getAutoCacheList(project = "ssr"):
    ...
    for item in workDB[project].find(query, sort=[("time", -1)]):
        if not shotDict.get(item["name"]) and os.path.exists(item["filepath"]) and not item['user'] in excludeNameList:
...
for expShow in showList:
    ...
    for key in autoCacheList.keys():
        ...
        if not sceneGraph.get("mayaVersion"):
            mayaVer = "2017"
        else:
        ...
        with open(jsonFile, "r") as f:
            ...
            if len(sceneGraph["components"]) >= 1:
                ...
                exportJob.addChild(meshJob)
            #
            # if sceneGraph.get('sim') and len(sceneGraph["sim"]) >= 1:
