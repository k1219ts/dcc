...
##########################################
__author__  = 'kwantae.kim in Dexter CGSupervisor'
__date__ = '2022.06.28'
...
class AssetImportTool(QtWidgets.QWidget):
                    ...
                    for assetName, variantPath in vsetSpec.variants.items():
                        if variantPath.variantSets.get('entity'): # entity
                            ...
                            for entityName in entityVSetSpec.variants.keys():
                                print('entityName:', assetName, entityName)
                                if entityName == 'asset':
                                    assetNameDir = os.path.join(os.path.dirname(assetUsdFilePath), assetName)
                                    print('assetNameDir:', assetNameDir)
                                else:
                                    shotDir = os.path.join('/show', self.ui.showEdit.text(), '_3d', 'shot')
                                    self.entityDict[assetName].append(entityName)

                                    coder = rb.Coder()
                                    argv = coder.N.SHOTNAME.Decode(entityName)
                                    seq = argv.seq
                                    if argv.shot:
                                        shot = coder.N.SHOTNAME.Encode(**argv)
                                        assetNameDir = os.path.join(shotDir, seq, shot, 'asset', assetName)
                                    else:
                                        assetNameDir = os.path.join(shotDir, seq, 'asset', assetName)
    ...
    def makeTreeWidgetFromAssetList(self):
        ...
        for assetName in sorted(self.assetList.keys()):
            ...
            elif self.assetList.get(assetName) and self.assetList[assetName]:
                # has branch
                isInsert = False
    ...
    def setItem(self,  item, dir, filename):
        versionItem = QtWidgets.QTreeWidgetItem(item, [filename, os.path.join(dir, filename)])
    ...
    def makeAssetItemList(self, assetDir, rootItem):
        # TASKS = ["model", "rig", "clip", "cam" ]
        # for task in TASKS:
        #     taskdir = os.path.join(assetDir, task)
        #     if os.path.exists(taskdir):
        #         taskitem = QtWidgets.QTreeWidgetItem(rootItem, [task])
        #         taskitem.setFont(0, self.itemFont)
        #
        #         if task == "model":
        #             for NS in os.listdir(taskdir):
        #                 if NS[0] in ['v', 'V']:
        #                     versionItem = self.setItem(rootItem, taskdir, NS)
        #                     versionDir = os.path.join(taskdir, NS)
        #
        #                     for file in os.listdir(versionDir):
        #                         if "_geom.usd" in name:
        #                             self.setItem(versionItem, versionDir, NS)
        #
        #         if task == "rig":
        #             for d in os.listdir(taskdir):
        #                 if not 'scenes' in d:
        #                     self.setItem(item, d)
        #                     lodList = ['low', 'mid']
        #                     for lod in lodList:
        #                         if os.path.exists(os.path.join(taskdir, 'scenes', i + "_%s.mb" % lod)):  # low check
        #                             filename = d + "_%s" % lod
        #                             file = os.path.join(taskdir,filename)
        #                             self.setItem(rootItem, d)
        ...
        if os.path.exists(os.path.join(assetDir, "lidar")):
            lidarItem = QtWidgets.QTreeWidgetItem(rootItem, ["lidar"])
            lidarItem.setFont(0, self.itemFont)

            lidarDir = os.path.join(assetDir, "lidar")
            for i in os.listdir(lidarDir):
                if os.path.isdir(os.path.join(lidarDir, i)):
                    if i[0] in ['v', 'V']:
                        versionItem = QtWidgets.QTreeWidgetItem(lidarItem, [i, os.path.join(lidarDir, i)])
                        versionItem.setFont(0, self.itemFont)
                        versionItem.setForeground(0, self.versionColor)

                        versionDir = os.path.join(lidarDir, i)
                        for j in os.listdir(versionDir):
                            if "_geom.usd" in j:
                                geomItem = QtWidgets.QTreeWidgetItem(versionItem, [j, os.path.join(versionDir, j), i])
                                geomItem.setFont(0, self.itemFont)
        ...
        if os.path.exists(os.path.join(assetDir, "cam")):
            camItem = QtWidgets.QTreeWidgetItem(rootItem, ["cam"])
            camItem.setFont(0, self.itemFont)

            camDir = os.path.join(assetDir, "cam")
            for i in os.listdir(camDir):
                if os.path.isdir(os.path.join(camDir, i)):
                    if i[0] in ['v', 'V']:
                        versionItem = QtWidgets.QTreeWidgetItem(camItem, [i, os.path.join(camDir, i)])
                        versionItem.setFont(0, self.itemFont)
                        versionItem.setForeground(0, self.versionColor)

                        versionDir = os.path.join(camDir, i)
                        for j in os.listdir(versionDir):
                            if ".geom.usd" in j:
                                geomItem = QtWidgets.QTreeWidgetItem(versionItem, [j, os.path.join(versionDir, j), i])
                                geomItem.setFont(0, self.itemFont)
    ...
    def assetItemDoublieClicked(self, assetNameItem, column):
        ...
        if self.entityDict.get(assetNameItem.text(0)):
    ...
    def rmbClicked(self, pos):
        ...
        elif item.parent() and "_lidar" in item.text(0):
            menu.addAction(QtGui.QIcon(), 'Maya Reference: {NAME}'.format(NAME=item.text(0)), lambda: self.referenceLidar(item))

        elif item.parent() and '_' in item.text(0):
    ...
    def referenceLidar(self, item):
        geomRes = item.text(0).split('_lidar.')[-1].split('.')[0]
        rootDir = os.path.dirname(item.text(1))
        print(geomRes, rootDir)

        refList = []
        for file in os.listdir(rootDir):
            if '.obj' in file and geomRes in file:
                refList.append(file)

        for ref in refList:
            assetName = ref.split('.')[0]
            assetFile = os.path.join(rootDir, ref)
            cmds.file(assetFile, r=True, type='obj', gl=True,
                                 mergeNamespacesOnClash=False,
                                 namespace=assetName, options='v=0;')
    ...
    def referenceMaya(self, item):
        ...
        m = self.XP_BRANCH.search(rootDir)
        namespace = m.group() if m else assetName
