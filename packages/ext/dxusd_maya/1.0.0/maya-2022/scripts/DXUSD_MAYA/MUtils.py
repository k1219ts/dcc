    ...
    if not os.path.exists(infoFile):
        ...
        if os.path.exists(pathRuleFile):
            with open(pathRuleFile, 'r') as f:
                ...
                if ruleData.get('refShow') and ruleData['refShow']:
...
def GetCameraNodes(node):
    cameras = []
    for c in cmds.listRelatives(node):
        if not c in cameras:
            cameras.append(c)

    return cameras

def PreReferenceAttr(node , featherFile, primpath ):
    import json
    usdattrJson = {
        "USD_prereferenceLayer": {
            "usdAttrName": "prereferenceLayer",
            "interpolation": "constant"
        },
        "USD_prereferencePrim": {
            "usdAttrName": "prereferencePrim",
            "interpolation": "constant"
        }
    }

    usdattrJson = json.dumps(usdattrJson)
    ...
    DATA = {
        ...
        'USD_UserExportedAttributesJson': {
            'niceName': 'User Exported Attributes Json',
            'value': usdattrJson
        },
        'USD_prereferencePrim': {
            'niceName': 'USD Prereference Prim',
            'value': primpath
        },
        'USD_prereferenceLayer': {
            'niceName': 'USD Prereference Layer',
            'value': featherFile
        }
    ...
    for attr in DATA.keys():
        label = DATA[attr]['niceName']
        val = DATA[attr]['value']
        if not cmds.attributeQuery(attr, n=node, exists=True):
            cmds.addAttr(node, ln=attr, nn=label, dt='string')

        cmds.setAttr('%s.%s' % (node, attr), val, type='string')
