...
class ModelExport(exp.ModelExporter):
    def Exporting(self):
        ...
        for i in range(len(self.arg.nodes)):
            ...
            # crowd asset export option
            elif '_crowd_GRP' in cmds.ls(node)[0] :
                crowd = utl.UsdExport(ofile, node)
                crowd.opts['exportSkels'] = "auto"
                crowd.opts['exportSkin'] = "auto"
                crowd.doIt()
                ...
                # abcExport
                abcfile = ofile.replace('.usd', '.abc')
                mutl.AbcExport(abcfile, [node])
    ...
    def Exporting(self):
        ...
        for i in range(len(self.arg.nodes)):
            ...
            for idx, mesh in enumerate(sorted(cmds.listRelatives(node, f=True))):
                ...
                geomFile = '%s_lidar_%s.%s_geom.obj' % (self.arg.asset, num, lod)
                exportPath = os.path.join(self.arg.dstdir, geomFile)
