...
def assetExport(node=None, show=None, shot=None, version=None, overwrite=False):
    ...
    #   Miarmy - MiarmyProForMaya$MAYA_VER.so
        ...
        assetExport_agent(node, show, shot, version, overwrite)
...
class GolaemAssetExport(exp.GolaemAssetExporter):
    def Exporting(self):
        ...
        # gcg query and copy
        geoFiles = cmds.glmCharacterFileTool(characterFile=self.arg.gcha,
                                             getAttr='geoFilePaths')
        worksGcgs = []
        for i in geoFiles:
            before = i.split(';')[-1]
            after = utl.SJoin(self.arg.dstdir, 'character', os.path.basename(before))
            msg.debug('### gcgCopy: %s -> %s' % (before, after))
            self.arg.gcgfiles.append(after)
            shutil.copy(before, after)

        # maya export
        cmds.select(self.arg.node)
        self.arg.pubscene = self.arg.gchafile.replace('.gcha', '.mb')
        cmds.file(self.arg.pubscene, pr=True,
    ...
    def GeomExport(self, filename):
        ...
        for i in self.meshes:
            if 'low' in i:  utl.SetPurposeAttribute([i], 'proxy')
            else:           utl.SetPurposeAttribute([i], 'render')
    ...
    def initializeGolaem(self):
        ...
        for glmCache in self.arg.glmCaches:
            ...
            gCache.customData = {'sceneFile': self.arg.scene,
                                 'gchaFiles': ', '.join(gCache.charFiles),
                                 'layoutFiles': ', '.join(gCache.layoutFiles)}
        ...
        for gCache in self.gCaches:
            ...
            for fieldName in gCache.fieldNames:
                ...
                for i in range(gCache.entityCount):
                    ...
                    # layout AddRemoveMeshAssets node
                    if gCache.removeMesh.get(entityId):
                        rMeshes = gCache.getRemoveMeshs(i)
                        if rMeshes:
                            allPrimPath = []
                            with utl.OpenStage(gCache.assetFiles[cid]) as stage:
                                for p in iter(Usd.PrimRange.AllPrims(stage.GetDefaultPrim())):
                                    if p.GetTypeName() == 'Mesh':
                                        tmp = p.GetPath().pathString.split('/')
                                        tmp[1] = spec.path.pathString
                                        primPath =  '/'.join(tmp[1:])
                                        allPrimPath.append(primPath)

                            for remove in rMeshes:
                                for prim in allPrimPath:
                                    if remove in os.path.basename(prim):
                                        activeSpec = utl.GetPrimSpec(outlyr, prim, specifier='over')
                                        activeSpec.SetInfo('active', False)
...
class agentAssetExport(exp.GolaemAssetExporter):
    def Exporting(self):
        # Member variable
        self.customData = {'sceneFile': self.arg.scene}
        self.meshes = cmds.listRelatives('geometry*')
        # self.meshes = cmds.glmCharacterFileTool(characterFile=self.arg.gcha,
        #                                         getAttr='meshAssets')
        bones = cmds.listRelatives('Skin*', type='joint')
        self.JOINT = GetGlmJoints(bones)
...
class GolaemShotComposit(exp.GolaemShotExporter):
    def Exporting(self):
        if self.arg.exportRange == [0, 0]:
            self.arg.exportRange = [self.arg.frameRange[0], self.arg.frameRange[1]]
        fps = mutl.GetFPS()
        srcfile = utl.SJoin(self.arg.dstdir, 'crowd.skel.*.usd')
        frameFiles = utl.GetPerFrameFiles(srcfile, self.arg.exportRange)
        utl.CoalesceFiles(frameFiles, self.arg.exportRange, fps, step=1.0, outFile=self.arg.skelfile)
...
def assetExport_agent(node=None, rootbone=None, show=None, version=None, overwrite=False):
    if not node:
        nodes = cmds.ls('glm_*')
        if not nodes:
            return
