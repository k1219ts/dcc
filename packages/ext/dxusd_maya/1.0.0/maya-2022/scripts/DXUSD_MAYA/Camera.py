...
class CameraGeomExport:
    def __init__(self, filename, node, fr=[None, None], step=0.0, customLayerData=dict()):
        ...
        if not self.customLayerData.get('sceneFile'):
    ...
    def doIt(self):
        ...
        attrHAO = Sdf.AttributeSpec(camSpec, 'horizontalApertureOffset', Sdf.ValueTypeNames.Float, Sdf.VariabilityVarying)
        self.SetAttribute(camSpec, attrHAO, self.node, 'horizontalFilmOffset', frames)
        ...
        attrVAO = Sdf.AttributeSpec(camSpec, 'verticalApertureOffset', Sdf.ValueTypeNames.Float, Sdf.VariabilityVarying)
        self.SetAttribute(camSpec, attrVAO, self.node, 'verticalFilmOffset', frames)
...
class PanZoom:
    ...
    def exportNukeCamShake(self, filename):
        ...
        nukeNode += '  inputs 0\n'
        # horizontalPan
        if self.shakeData.get('horizontal'):
            tx = '{curve'
            ...
            tx = '%.8f' % (value / hfa * 2)
        # verticalPan
        if self.shakeData.get('vertical'):
            ty = '{curve'
        ...
        for i in range(len(self.arg.nodes)):
            ...
            for dxnode in self.arg.dxnodes:
                ...
                if camGeo:
                    ...
                    if cmds.keyframe(dxnode, q=True):
                        DummyExport(camgeoFile, camGeo[0], fr=frameRange, step=self.arg.step, abcExport=abcExport).doIt()
                    else:
...
def cameraExport(dxNodes=[], abcExport=True, overwrite=False,
                 show=None, seq=None, shot=None, version=None, user=None, fr=[0, 0], step=0.0, process='geom', insertDb=False):
    if not dxNodes:
        dxNodes = cmds.ls(type='dxCamera')
    ...
    if shot: arg.ovr_shot = shot
    if version: arg.ovr_ver = version
    if insertDb: arg.insertDb = insertDb
