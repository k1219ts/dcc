...
class RigNodeInspect:
    ...
    def keyFrameChecking(self):
        ...
        for ctrl in self.ctrls:
            for node in cmds.listHistory(ctrl, lf=False):
                if cmds.nodeType(node).find('animCurve') > -1: # animCurveLU LT
                    ...
                    if frames:
                        allFrames += frames
                elif cmds.nodeType(node).find('parentConstraint') > -1:
                    self.constraint = True

                # expression animation mesh cache export
                try:
                    for connNode in cmds.listConnections(node):
                        if 'expression' in connNode:
                            msg.debug('#### find Expression: %s' % connNode)
                            self.checkDeform = True
                            break
                except:
                    pass
        ...
        # deformed mesh cache export
        for mesh in cmds.ls(self.objects['high'], dag=True, type='mesh'):
            for n in cmds.listHistory(mesh):
                if 'time' in n or 'blend' in n:
                    self.checkDeform = True
                    break

        if not self.checkDeform and not allFrames and self.rigType != 4 and not self.constraint: # if rigType isn't etc and not set keyframe, export just frame
    ...
    def bake(self):
        ...
        for c in self.ctrls:
            ...
            if tangents:
                ...
                if len(tangents) > 1:
                    _valid += 1
        if not self.checkDeform and _valid == 0 and self.rigType != 4 and not self.constraint: # if rigType isn't etc
            self.exportRange = (self.fr[0], self.fr[0])
...
class RigAssetExport(exp.RigAssetExporter):
    ...
    def Exporting(self):
        ...
        if not ctrlData:
            msg.errorQuit('not found controller attribute.', artist='kwantae.kim')
        # 2. Meshes exists
        self.updateMeshes()
        ...
        print('#### self.arg.variant:', self.arg.variant)
        if self.arg.get('branch'):
        # if self.arg.variant != "":
            ...
            for geomGrpChild in geomGrpChildren:
                if '_model_' not in geomGrpChild:
                    ...
                    if '%s_%s_' % (self.arg.asset, self.arg.branch) in geomGrpChild:
                        ...
                        print('### delete geomGrpChild:', geomGrpChild)
        ...
        for lodType in lodTypeList:
            ...
            if lodType == 'high':
                ...
                self.arg.pubscene = pubSceneFile
...
class RigShotGeomExport(exp.RigShotBase):
    def preProcess(self):
        if not cmds.objExists(self.arg.node):
            msg.errorQuit('not found rig node [%s]' % self.arg.node, artist='kwantae.kim')
        ...
        if cmds.referenceQuery(self.arg.node, inr=True):
            ...
            if not '/_3d/' in refFile:
                msg.errorQuit('[USD Asset]', "Rig asset not publish data =>", 'Rig File : %s' % refFile, artist='kwantae.kim')
        ...
        if os.path.exists(arg.D.TASKN):
            ...
            msg.errorQuit('[USD Asset]', 'First, you have to export RigAsset', 'Rig File : %s' % rigFile, 'RigAssetDir : %s' % arg.D.TASKN, artist='kwantae.kim')

    def preCompute(self):
        self.rigInspector = RigNodeInspect(self.arg.node, self.arg.frameRange, self.arg.autofr)
        self.rigInspector.bake()
        print('################### rigInspector.exportRange:', self.rigInspector.exportRange)
    ...
    def geomExport(self, filename, objects):
        ...
        try:
            abcfile = filename.replace('.usd', '.abc')
            mutl.AbcExport(abcfile, objects, fr=self.arg.frameRange, step=self.arg.step)
            self.arg.abcFiles.append(abcfile)
        except:
            pass

    def subframe_geomExport(self, filename, objects, frameStride, fileStep):
        ovr_opts = {'exportUVs': False,
                    'frameStride': frameStride}

        for start, end in mutl.GetIterFrames(self.arg.frameRange, step=fileStep):
            fn = filename.replace('.usd', '.%04d.usd' % start)
            utl.UsdExport(fn, objects, fr=(start, end), **ovr_opts).doIt()

            try:
                abcfile = fn.replace('.usd', '.abc')
                mutl.AbcExport(abcfile, objects, fr=(start, end), step=fileStep)
                self.arg.abcFiles.append(abcfile)
            except:
                pass
    ...
    def computeSeparateFrames(self, geomFile, objects, lod):
        ...
        totalSize = (refSize * numframes) * 0.1
        limitSize = 20.0 # GB
    ...
    def Exporting(self):
        ...
        if objects['high']:
            ...
            # get mesh txVarNum
            for mesh in cmds.ls(exportObjects, dag=True, l=True, type='mesh'):
                if cmds.attributeQuery('rman__riattr__user_txVarNum', n=mesh, ex=True):
                    varNum = int(cmds.getAttr('%s.rman__riattr__user_txVarNum' % mesh))
                    parent = cmds.listRelatives(mesh, p=True, f=True)[0]
                    n = parent.replace('|%s:' % self.arg.namespace, '/')
                    self.arg.txVarNum.append([n.replace('|', '/').replace(':', '_'), varNum])
        if objects['mid']:
            # MID GEOM
            self.arg.lod = var.T.MID
            ofile = utl.SJoin(self.arg.D.TASKNV, self.arg.F.GEOM)
        ...
        if cmds.getAttr('%s.exportStep' % self.arg.node):
            ...
            for gt, objs in subobjects.items():
                if objs:
                    ...
                    self.subframe_geomExport(ofile, objs, step, filestep)
        ...
        # mayaScene Save as
        self.arg.pubscene = utl.SJoin(utl.DirName(self.arg.dstdir), 'scenes', self.arg.F.MAYA.BASE)
