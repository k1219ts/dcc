...
class AModelExporter(AExport):
    def __init__(self, **kwargs):
        ...
        self.abcfiles  = []     # export geom alembic filename.
        self.master    = ''
    ...
    def Treat(self):
        ...
        self.N.MAYA.SetDecode(self.nodes[0])
        ...
        try:
            self.pubscene = utl.SJoin(self.D.OTHER.TASKSCENE, self.F.MAYA.BASE.replace('_GRP', ''))
        except:
            pass
...
class ModelExporter(Export):
    ...
    def Tweaking(self):
        ...
        # 3 For Vendor
        twks = twk.Tweak()
        self.arg.texAttrUsd = self.tArg.texAttrUsd
        twks << twk.InsertDb(self.arg)                      # pub history DB
        twks << twk.SaveMayaScene(self.arg)                 # assignTexture & saveAs MayaScene
        twks.DoIt()
