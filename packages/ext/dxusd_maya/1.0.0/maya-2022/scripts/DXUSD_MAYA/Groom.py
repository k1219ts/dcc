...
def GetGroomFile(rigFile, variant=None):    # rigFile is *.usd or *.mb
    ...
    if not spec:
        primPath = Sdf.Path('/' + srclyr.defaultPrim)
        rig_version = rig_version.split('_')
        rig_version[-1] = utl.Ver(0)
        rig_version = '_'.join(rig_version)
        primPath = primPath.AppendVariantSelection(var.T.VAR_RIGVER, rig_version)
        spec = srclyr.GetPrimAtPath(primPath)
        if not spec:
            ...
            msg.warning('Groom.GetGroomFile >>> Use %s rig version.'%rig_version)

    gfile = None
    vsetSpec = spec.variantSets.get(var.T.VAR_GROOMVER)
    if vsetSpec:
        ver = spec.variantSelections[var.T.VAR_GROOMVER]
        gfile = utl.SJoin(arg.D.TASK, 'scenes', ver + '.mb')
        arg.nslyr = ver
    ...
    if gvlyr.customLayerData.get('sceneFile') and gvlyr.customLayerData['sceneFile'].endswith('.hip'):
...
def GetConfigGroomFile(rigFile, outDir, nslyr=None):
    # outDir '/show/slc/_3d/shot/MET/MET_0010'
    msg.debug('Groom.GetConfigGroomFile >>> rigFile :', rigFile)
    arg = Arguments()
    arg.D.SetDecode(utl.DirName(rigFile))
    arg.D.SetDecode(outDir)
    seq = arg.seq
    shotName = arg.N.SHOTNAME

    infoFile = '{DIR}/_config/maya/AssetInfo.json'.format(DIR = arg.D.SHOW)
    if os.path.isfile(infoFile):
        with open(infoFile, 'r') as f:
            data = eval(f.read())
            if 'groom' in data:
                if arg.asset in data['groom']:
                    for k, v in data['groom'][arg.asset].items():
                        info = data['groom'][arg.asset][k]
                        if seq in info['seq'] or shotName in info['shot']:
                            groomFile = info['groomFile']
                            if os.path.exists(groomFile):
                                msg.debug('Groom.GetConfigGroomFile >>> rigFile :', groomFile)
                                return groomFile
                else:
                    return False
    else:
    ...
    def Exporting(self):
        ...
        for n in self.arg.groom_nodes:
            if cmds.attributeQuery('MaterialSet', n=n, ex=True):
                ...
                if mtlname:
                    if not self.arg.groomAttrData.get(n):
...
class GroomShotGeomExport(exp.GroomShotExporter):
    ...
    def MergeExport(self):
        ...
        # Rig Check
        for rig in cmds.ls(type='dxBlock', l=True):
            if 'ZN' in rig:
                self.arg.rigNode = rig
                break
