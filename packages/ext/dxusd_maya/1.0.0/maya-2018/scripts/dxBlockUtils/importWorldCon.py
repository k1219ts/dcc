...
class importWorldCon:
    ...
    def setRootConInitPosition(self):
        ...
        rootConMultMatrixInit = '%s_rootCon_multMatrixInit'%self.name
        if len(cmds.ls(rootConMultMatrixInit)) < 1:
        ...
        rootConMatrixInit = '%s_rootCon_matrixInit'%self.name
        if len(cmds.ls(rootConMatrixInit)) < 1:
            rootConMatrixInit = cmds.shadingNode('fourByFourMatrix', asUtility=True, n='%s_rootCon_matrixInit'%self.name)

        rootConInverseMatrixInit = '%s_rootCon_inverseMatrixInit'%self.name
        if len(cmds.ls(rootConInverseMatrixInit)) < 1:
        ...
        rootConInitPosNUL = '%s_rootConInitPos_NUL'%self.name
        if len(cmds.ls(rootConInitPosNUL)) < 1:
            rootConDecomposeMatrixInit = '%s_rootCon_decomposeMatrixInit'%self.name
            if len(cmds.ls(rootConDecomposeMatrixInit)) < 1:
        ...
        matrixInit = '%s_matrixInit'%self.name
        if len(cmds.ls(matrixInit)) < 1:
            matrixInit = cmds.shadingNode('fourByFourMatrix', asUtility=True, n='%s_matrixInit'%self.name)

        cmds.setAttr('%s.in00'%matrixInit, cmds.getAttr('%s_matrix.in00'%self.name, t=startTime) * cmds.getAttr('%s_matrix.initScale'%self.name, t=startTime))
        cmds.setAttr('%s.in01'%matrixInit, cmds.getAttr('%s_matrix.in01'%self.name, t=startTime) * cmds.getAttr('%s_matrix.initScale'%self.name, t=startTime))
        cmds.setAttr('%s.in02'%matrixInit, cmds.getAttr('%s_matrix.in02'%self.name, t=startTime) * cmds.getAttr('%s_matrix.initScale'%self.name, t=startTime))
        cmds.setAttr('%s.in03'%matrixInit, cmds.getAttr('%s_matrix.in03'%self.name, t=startTime) * cmds.getAttr('%s_matrix.initScale'%self.name, t=startTime))
        cmds.setAttr('%s.in10'%matrixInit, cmds.getAttr('%s_matrix.in10'%self.name, t=startTime) * cmds.getAttr('%s_matrix.initScale'%self.name, t=startTime))
        cmds.setAttr('%s.in11'%matrixInit, cmds.getAttr('%s_matrix.in11'%self.name, t=startTime) * cmds.getAttr('%s_matrix.initScale'%self.name, t=startTime))
        cmds.setAttr('%s.in12'%matrixInit, cmds.getAttr('%s_matrix.in12'%self.name, t=startTime) * cmds.getAttr('%s_matrix.initScale'%self.name, t=startTime))
        cmds.setAttr('%s.in13'%matrixInit, cmds.getAttr('%s_matrix.in13'%self.name, t=startTime) * cmds.getAttr('%s_matrix.initScale'%self.name, t=startTime))
        cmds.setAttr('%s.in20'%matrixInit, cmds.getAttr('%s_matrix.in20'%self.name, t=startTime) * cmds.getAttr('%s_matrix.initScale'%self.name, t=startTime))
        cmds.setAttr('%s.in21'%matrixInit, cmds.getAttr('%s_matrix.in21'%self.name, t=startTime) * cmds.getAttr('%s_matrix.initScale'%self.name, t=startTime))
        cmds.setAttr('%s.in22'%matrixInit, cmds.getAttr('%s_matrix.in22'%self.name, t=startTime) * cmds.getAttr('%s_matrix.initScale'%self.name, t=startTime))
        cmds.setAttr('%s.in23'%matrixInit, cmds.getAttr('%s_matrix.in23'%self.name, t=startTime) * cmds.getAttr('%s_matrix.initScale'%self.name, t=startTime))
        cmds.setAttr('%s.in30'%matrixInit, cmds.getAttr('%s_matrix.in30'%self.name, t=startTime))
        cmds.setAttr('%s.in31'%matrixInit, cmds.getAttr('%s_matrix.in31'%self.name, t=startTime))
        cmds.setAttr('%s.in32'%matrixInit, cmds.getAttr('%s_matrix.in32'%self.name, t=startTime))
        cmds.setAttr('%s.in33'%matrixInit, cmds.getAttr('%s_matrix.in33'%self.name, t=startTime))

        inverseMatrixInit = '%s_inverseMatrixInit'%self.name
        if len(cmds.ls(inverseMatrixInit)) < 1:
            inverseMatrixInit = cmds.shadingNode('inverseMatrix', asUtility=True, n='%s_inverseMatrixInit'%self.name)
        cmds.connectAttr('%s.output'%matrixInit, '%s.inputMatrix'%inverseMatrixInit, force=True)

        worldRootConMultMatrixInit = '%s_world_rootCon_multMatrixInit'%self.name
        if len(cmds.ls(worldRootConMultMatrixInit)) < 1:
            worldRootConMultMatrixInit = cmds.shadingNode('multMatrix', asUtility=True, n='%s_world_rootCon_multMatrixInit'%self.name)
        cmds.connectAttr('%s.output'%rootConMatrixInit, '%s.matrixIn[0]'%worldRootConMultMatrixInit, force=True)
        cmds.connectAttr('%s.outputMatrix'%inverseMatrixInit, '%s.matrixIn[1]'%worldRootConMultMatrixInit, force=True)
        cmds.connectAttr('%s.matrixSum'%worldRootConMultMatrixInit, '%s.inputMatrix'%rootConInverseMatrixInit, force=True)
        cmds.connectAttr('%s_rootCon_localMatrix_multMatrix.matrixSum'%self.name, '%s.matrixIn[0]'%rootConMultMatrixInit, force=True)
