...
class RigAssetExport(exp.RigAssetExporter):
    ...
    def Exporting(self):
        ...
        if not ctrlData:
            msg.errorQuit('not found controller attribute. %s' % self.arg.node, artist='kwantae.kim')
        # 2. Meshes exists
        self.updateMeshes()
...
class RigShotGeomExport(exp.RigShotBase):
    def preProcess(self):
        if not cmds.objExists(self.arg.node):
            msg.errorQuit('not found rig node [%s]: %s' % (self.arg.node, self.arg.user), artist='kwantae.kim')
        ...
        if cmds.referenceQuery(self.arg.node, inr=True):
            ...
            if not '/_3d/' in refFile:
                msg.errorQuit('[USD Asset]', "Rig asset not publish data =>", 'Rig File : %s, %s' % (refFile, self.arg.user), artist='kwantae.kim')
