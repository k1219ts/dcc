...
)
{
    ...
    _skelCache.Populate(skelRoot,UsdTraverseInstanceProxies());

    std::vector<UsdSkelBinding> bindings;
    if (!_skelCache.ComputeSkelBindings(skelRoot, &bindings,UsdTraverseInstanceProxies())) {
        return;
    }
    ...
    for (const UsdSkelBinding &binding : bindings)
    {
        ...
        if (const UsdSkelSkeletonQuery &skelQuery = _skelCache.GetSkelQuery(binding.GetSkeleton())) {
            ...
            for (double relSampleTime : motionSampleTimes) {
                ...
                float correctedSampleTime = isMotionBackward ? UsdKatanaUtils::ReverseTimeSample(relSampleTime) : relSampleTime;
            ...
            if (!xformsTimeSampleMap.empty())
            {
                for (UsdSkelSkinningQuery const &skinQuery : binding.GetSkinningTargets()) {
                    ...
                    if (!pointsTimeSampleMap.empty())
                    {
                        ...
                        FnKat::DoubleAttribute boundsAttr = UsdKatanaUtils::ConvertBoundsToAttribute(bounds, motionSampleTimes, isMotionBackward, &hasInfiniteBound);
