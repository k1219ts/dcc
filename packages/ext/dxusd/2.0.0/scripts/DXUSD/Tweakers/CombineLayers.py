...
class ACombineLayers(ATweaker):
    def __init__(self, **kwargs):
        ...
        rules (list) : rules of source and destination prim
            ...
            - source rule : None - use default prim (/로 시작>MatchPath or MatchName)
            ...
            - destination rule :
                None - use same prim path
                'prim path' - move prim under the given path
                '/' - source prim name is the root prim (/abc/def > /def)
                '/.'- source prim's root name is the root prim (/abc/def > /abc)
    ...
    def Treat(self):
        ...
        # copy meta data from source layer[0]
        self.meta.Get(self.inputs[0])
...
class CombineLayers(Tweaker):
    ...
    def DoIt(self):
        ...
        # dstlyr   = utl.AsLayer(self.arg.outputs[0], create=True, clear=True)
        ...
        if self.arg.dprim and dstlyr.GetPrimAtPath(self.arg.dprim):
        ...
        if self.arg.flatten:
            with utl.OpenStage(dstlyr) as stg:
                stg.Flatten()
                stg.Save()
                stg.Export(dstlyr.realPath)
        ...
        with utl.OpenStage(srclyr) as stg:
            ...
            if rules.dprim:
                for rule in rules.dprim:
                    res.append(self.GetRefSpec(srcrelpath, rule[1] or dprim))
            else:
    ...
    def GetRefSpec(self, relpath, dst, src=Sdf.Path.emptyPath):
        ref = Sdf.Reference(relpath, str(src or Sdf.Path.emptyPath))
        return (ref, dst)
            ...
            with utl.OpenStage(dstlyr) as stage:
                ...
                for p in treeIter:
                    if p.GetName() == spec.name:
                        if self.isSeq:
                            tmplyr = utl.AsLayer(self.arg.dependHighGroom)
                            for tspec in tmplyr.rootPrims[0].nameChildren:
                                with utl.OpenStage(tmplyr) as stage:
                                    tIter = iter(Usd.PrimRange.AllPrims(stage.GetPseudoRoot()))
                                    tIter.next()
                                    for tp in tIter:
                                        if tp.GetName() == spec.name:
                                            p = tp
