...
class CombineGroomLayers(Tweaker):
    ...
    def curvesAttributes(self, spec):
        ...
        if self.arg.has_attr('groomAttrData') and self.arg.groomAttrData.get(spec.name):
    ...
    def bodyMeshAttributeClass(self, outlyr, data):
        if not (data.get(var.T.TXBASEPATH) and data.get(var.T.TXLAYERNAME)):
            return
        basePath  = data[var.T.TXBASEPATH]
        layerName = data[var.T.TXLAYERNAME]
        modelVer  = 'v001'
        if data.get(var.T.MODELVER):
