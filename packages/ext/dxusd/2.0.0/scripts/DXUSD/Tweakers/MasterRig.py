...
class MasterRigPack(Tweaker):
    ...
    def DoIt(self):
        ...
        # }
        if not self.geomfiles.get('high'):
        ...
        self.rigCollection = ''
        self.rigAssetName  = ''
        if customData.get('rigFile') and customData['rigFile'] != '':
    ...
    def main(self, outlyr):
        ...
        if self.geomfiles.get('mid') or self.geomfiles.get('low'):
            # LOD Variant
            ...
            # mid
            if self.geomfiles.get('mid') and self.geomfiles['mid']:
                vspec = Sdf.VariantSpec(vsetSpec, var.T.MID)
                spec  = utl.GetPrimSpec(outlyr, vspec.primSpec.path.AppendChild('Geom'))
                self.geomArc(spec, self.geomfiles['mid'])
            # low
            if self.geomfiles.get('low') and self.geomfiles['low']:
