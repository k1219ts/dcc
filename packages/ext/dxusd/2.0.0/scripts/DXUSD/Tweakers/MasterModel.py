...
class AMasterModelPack(ATweaker):
    ...
    def Treat(self):
        ...
        if not self.has_attr('dstdir') or not self.dstdir:
        ...
        for f in files:
            for gtyp in [var.T.HIGH, var.T.MID, var.T.LOW]:
                if '%s_geom.usd' % gtyp in f:
                    if not self.geomfiles.get(gtyp):
...
class MasterModelPack(Tweaker):
    ARGCLASS = AMasterModelPack
    def DoIt(self):
