...
class Collection(Tweaker):
    ...
    def composite(self, output, input, remove=False):
        ...
        source = utl.GetRelPath(outlyr.identifier, input)
        source = Sdf.Reference(source)
        print(source)
    ...
    def DoIt(self):
        ...
        if self.arg.inputRigFile:
            output = self.arg.inputRigFile.replace('.usd', '.collection.usd')
            ...
            if self._ifpreview > 0:
                ...
                if spec is None:
                    spec = utl.GetPrimSpec(outlyr, lookSpec.path.AppendChild('preview'), type='Scope')
    ...
    def createPreviewRoot(self):
        ...
        if not vsetSpec.variants.get('off'):
            vspec = Sdf.VariantSpec(vsetSpec, 'off')
        if not vsetSpec.variants.get('on'):
