...
def GetDefaultPrim(layer, sublayer=False):
    ...
    if not spec and layer.rootPrims:
        spec = layer.GetPrimAtPath(layer.rootPrims[1])
    ...
    if not spec and sublayer:
        for sub in layer.subLayerPaths:
            ...
            if not sub or sub.defaultPrim:
                continue
            spec = sub.GetPrimAtPath(sub.defaultPrim)
            if spec:
                break
...
def SetSublayer(layer, src, idx=0):
    ...
    if isinstance(src, Sdf.PrimSpec):
        ...
        src = src.realPath
        src = GetRelPath(layer.identifier, src)
...
class UpdateLayerData:
    ...
    def timecode(self):
        if self.srclyr and (self.srclyr.startTimeCode or self.srclyr.endTimeCode):
            ...
            if self.srcdata.get('start'):
                start = self.srcdata['start']
            if self.srcdata.get('end'):
            ...
            print('self.srclyr:', self.srclyr)
            print('self.outlyr:', self.outlyr)
            print('srclyr(from) fps:', self.srclyr.framesPerSecond)
            print('outlyr(to) fps:', self.outlyr.framesPerSecond)
...
PathExists = lambda *args: os.path.exists(args[0])

# Ver      = lambda *args: 'v%03d'%(args[0])
IsVer    = lambda *args: var.rb.MatchFlag('ver', args[0])
# VerAsInt = lambda *args: int(re.search('\d{%s}' % VerDigit(), args[0]).group())
    ...
    with open(jsonPath) as jsn:
        ...
        for d in shares:
            if d.startswith(var.SEP):
                ...
                if prs.get('show') and not prs.get('pub'):
...
def GetAbsPath(current, target):
    ...
    curdir = os.path.dirname(current) if isfile(current) else current
    return os.path.abspath('%s/%s'%(curdir, target))
...
def checkUnicode(path):
    try:    return isinstance(path, (str, unicode))
    except: return isinstance(path, str)
