...
and finding nods that are relevant to the submission
...
class SubmitterROP(object):
    ...
    def saveHipFile(self):
        ...
        try:
            ...
            hnproc = SubmitterPreProc.HouNodeProc()
            hnproc.disableSimCacheOpencl()
            hou.hipFile.save(file_name=self.hipFile, save_to_recent_files=False)
            hnproc.undoDisableSimCacheOpencl()
    ...
    def hfsJobScript( self ):
        farmsel = self.sub.parm("selection").eval()
        if farmsel == 0:
            farmsel = 'Houdini'
        elif farmsel == 2:
            farmsel = 'Houdini19'
        elif farmsel == 1 :
            farmsel = 'storm | simfarm'
        else :
            farmsel = 'Houdini19-IT'
            print("IT")
    ...
    def Create_Task(self, taskname, subtasks, parentTask, tasktype):
        if not taskname in self.m_taskMap:
    ...
    def Create_FetchTask(self, node, parentTask):
        ...
        # 3. Add ROP Task
        if not task_name in self.m_ropMap:
    ...
    #---------------------------------------------------------------------------
    def submit( self ):
        ancestor = self.sub.inputAncestors()[0] 
        ancestorType = str(ancestor.type())
        
        if "Mantra" in ancestorType : 
            camVerify = self.sub.inputAncestors()[0].parm("camera").eval()
            if camVerify == "/obj/cam1" :
                message = "카메라가 " + camVerify + " 입니다."
                hou.ui.displayMessage(message, severity=hou.severityType.Warning)
                pass
            else :
                bgImageVerify = hou.node(camVerify).parm("vm_bgenable").eval()
                if bgImageVerify == 1 :
                    hou.ui.displayMessage("배경 이미지 랜더가 켜져있습니다.", severity=hou.severityType.Warning)
                    pass

            
            mblurCheck = ancestor.parm("allowmotionblur").eval()
            if mblurCheck == 0 :
                ancestor.setComment("모션블러가 꺼져있습니다.")
                ancestor.setGenericFlag(hou.nodeFlag.DisplayComment,True)
            else : 
                ancestor.setComment("")
                ancestor.setGenericFlag(hou.nodeFlag.DisplayComment,False)

        elif "Mantra" not in ancestorType :
            pass
