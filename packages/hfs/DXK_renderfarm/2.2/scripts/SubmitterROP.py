...
class SubmitterROP(object):
    def __init__( self, submitterNode ):
        ...
        self.tractorEngine = TractorEngine()
    ...
    def houdiniParms(self):
        ...
        # environment key
        envkey = 'hfs%s' % os.getenv('HOUDINI_VERSION')
        bundle = os.getenv('BUNDLE_NAME')
        if bundle:
            envkey += '=' + bundle
        self.envkey = [envkey]
    ...
    def Create_Task(self, taskname, subtasks, parentTask):
    ...
    def Create_FetchTask(self, node, parentTask):
        ...
        self.Create_Task(task_name, 1, parentTask)
    ...
    def Create_ICollectTask(self, node, parentTask):
        ...
        self.Create_Task(task_name, 0, parentTask)
        task = self.m_taskMap[task_name]
    ...
    def connectROP( self, fetchNode, parentTask ):
        if not fetchNode.isBypassed():
            rop = ROP.ROP( fetchNode, parentTask, hipFile=self.hipFile, envKey = self.envkey )
