#
#
# 3DE4.script.name:  Export Undistorted Images for MMV ...
#
# 3DE4.script.version:  v2.7
#
...
# Edit by kwantae.kim(MMV) 180907
...
# v2.5.1: minor change version, script name
# v2.6: overscan value
# v2.7: show_config added (colorspace)
#
...
NUKE_PATH = '/backstage/apps/Nuke/Team_MMV'
NUKE_EXEC_PATH = 'Cannot Find Nuke.'
FONT_PATH = '/backstage/apps/3DEqualizer/py_scripts/distortion/Vera.ttf'
OVERSCAN = 1.08
...
if NUKE_VER:
    NUKE_EXEC_PATH = '/opt/Nuke%s/Nuke%s'%(NUKE_VER, NUKE_VER.split("v")[0])
if not NUKE_PATH:
...
def _undistort_images_callback(requester, widget, action):
    global OVERSCAN
    #print widget, OVERSCAN
    # OVERSCAN CHECK SETTING
    if widget == 'overscan':
    ...
    # OVERSCAN VALUE COMBOBOX AND CUSTOM SETTING
    if widget == 'overscan_v':
        overscan_index = tde4.getWidgetValue(req, "overscan_v")
    ...
    if widget == 'overscan_custom':
        OVERSCAN = float(tde4.getWidgetValue(req, "overscan_custom"))

    # OVERSCAN WIDTH HEIGHT SETTING
...
window_title = 'Export Undistorted Images for MMV %s...'%VERSION
...
if not proj_path:
    ...
    for cam in cam_list:
        if tde4.getCameraProxyFootage(cam) == 0:
            ...
            seq_colorspace_R = DD_common.get_show_config(show, 'colorspace_R') # result: 'AlexaV3LogC', 'linear'...
            seq_colorspace_W = DD_common.get_show_config(show, 'colorspace_W') # result: 'AlexaV3LogC', 'linear'...
            if seq_colorspace_R == []: seq_colorspace_R = 'default' # plate default value (ex: Cineon)
            if seq_colorspace_W == []: seq_colorspace_W = 'sRGB' # sRGB is default
            ...
            tde4.setWidgetCallbackFunction(req, 'overscan_v', '_undistort_images_callback')
            tde4.setWidgetSensitiveFlag(req, "overscan_v", 0)

            tde4.addTextFieldWidget(req, "overscan_custom", "Overscan Custom", '')
            tde4.setWidgetSensitiveFlag(req, "overscan_custom", 0)
            tde4.setWidgetCallbackFunction(req, 'overscan_custom', '_undistort_images_callback')
            ...
            overscan_scale = round(bbox[2] / seq_width, 2)
            if overscan_scale < round(bbox[3] / seq_height, 2):
                overscan_scale = round(bbox[3] / seq_height, 2)
            ...
            tde4.addTextFieldWidget(req, 'colorspace_R', 'Plate colorspace', str(seq_colorspace_R))
            tde4.addTextFieldWidget(req, 'colorspace_W', 'Output colorspace', str(seq_colorspace_W))
            tde4.setWidgetSensitiveFlag(req, 'colorspace_R', 0)
            tde4.setWidgetSensitiveFlag(req, 'colorspace_W', 0)
            ...
            if ret==1:
                ...
                for frame in range(1, nfr+1):
                    ...
                    try:
                        ...
                        f.write('seq = nuke.nodes.Read(file=\'%s\', format=\'%d %d 0 0 %d %d 1\', first=%d, last=%d, colorspace=\'%s\')\n\n'%(cam_path,
                                 seq_width, seq_height, seq_width, seq_height, frame+(seq_attr[0]-1), frame+(seq_attr[0]-1), seq_colorspace_R))
                ...
                if only_script == 0:
                    ...
                    if overscan == 1:
                        ...
                        record['task_publish']['overscan_value'] = overscan_v
            ...
            tde4.postQuestionRequester(window_title, 'Change the Froxy Footage to "Main"', 'Ok')
