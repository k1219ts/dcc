#
#
# 3DE4.script.name:  3. dxImport Project Lens...
#
...
def _import_lens_callback(requester, widget, action):
    ...
    if widget == 'lensList':
        ...
        lensList = DD_common.getFileList(os.path.join(lensPath, slLens), '', '*.txt')
...
if 'show' in os.environ:
    ...
    lensPath = os.path.join('/', 'show', os.environ['show'], 'works', 'MMV', 'asset', 'lens')
    ...
    if ret==1:
        ...
        path = os.path.join('/', 'show', os.environ['show'], 'works', 'MMV', 'asset', 'lens', sllLens, slRes)
        #print(path)
