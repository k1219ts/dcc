...
class dxImportDummy:
    def __init__(self, requester):
        ...
        self.mmvDummyPath = os.path.join(self.SHOW_ROOT, self.show, 'works', 'MMV', 'asset', 'dummy')
    ...
    def _assetListCallback(self, requester, widget, action):
        ...
        if asset in ['mmvDummy', 'global']:
            dummyList = DD_common.getDummyList(self.mmvDummyPath)
    ...
    def getPath(self):
        ...
        if asset in ['mmvDummy', 'global']:
            path = os.path.join(self.mmvDummyPath, slDummy)
        else:
            tmp = os.path.join(self.assetPath.format(asset=asset), assetType)
            ver = DD_common.GetLastVersion(tmp)
