...
def setShowConfig(show):
    ...
    if os.path.exists(showRbPath):
        ...
        if 'DXRULEBOOKFILE' in os.environ:
            del os.environ['DXRULEBOOKFILE']

def checkProject(type='show'):
    if 'show' == type:
        if 'show' in os.environ and os.environ['show'] != '':
            return True
        else:
...
def getMyTask(user=getpass.getuser()):
    ...
    info2 = requests.get("http://%s/dexter/search/task.php" % dxConfig.getConf('TACTIC_IP'), params=params).json()
...
def getSeqFileList(dirPath):
    ...
    seqList = [getSeqFile(tuple(group)) \
        for key, group in itertools.groupby(enumerate(DATA), lambda index_name: index_name[0] - int(extractNumber(index_name[1])))]
    return seqList
