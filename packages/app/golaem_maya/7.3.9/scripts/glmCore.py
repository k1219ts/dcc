...
class ReadSimCache:
            ...
            for gscl in self.layoutFiles:
                ...
                for i in infos['nodes']:
                    if i['name'] == 'Kill' and i['active'] != 0:
                        ...
                        infos['tmp_value'] = i['attributes'][-1]['values'][0]
                        connection = self.listNodeConnections(infos, i['ID'])
    ...
    def doIt(self, fieldName):
        ...
        for f in self.charFiles:
            ...
            self.charsBoneIds.append(boneIds)
            boneIds = glmapi.intArray_frompointer(glmapi.getParentBones(f))
            self.charsParentBoneIds.append(boneIds)
