...
def _set_python_paths(rmantree):
    ...
    # build python version string for site-packages
    pyvers = "python%d.%d" % (sys.version_info.major, sys.version_info.minor)
    site_pkgs = None
    ...
    # PySide2 is in our python path ($RMANTREE/lib/<pyvers>/site-packages/PySide2)
    # which means rman_utils.vendor.Qt (used by rman_assets.ui) will find and import 
    # import PySide2 before it can pick up PyQt5. Katana requires PyQt5 so the 
    # environment is set up here to specify a different binding order in Qt.py
    os.environ["QT_PREFERRED_BINDING_JSON"] = r'{"rman_utils.vendor.Qt": ["PyQt5", "PySide2"]}'
...
def create_displayFilters(Asset, assignToSelected):
    ...
    for node in nodeList:
        outPort = node.getOutputPortByIndex(0)
        SNAC.addInputPortAtIndex('i'+str(idx), idx)
    ...
    outPort = combinerNode.getOutputPortByIndex(0)
    outPort.connect(dstPort)
...
# lnrTd @2024.02.14
def import_envmap(Asset, filepath, lightName=None):
    ...
    envName = Asset.envMapName()

    if os.path.splitext(envName)[1] == '.tex':
        envMapPath = Asset.envMapPath()
    elif os.path.splitext(envName)[1] == '.exr':
        envMapPath = os.path.join(os.path.dirname(filepath), envName)
...
class PrmanPresetBrowser(UI4.Tabs.BaseTab):
...
PluginRegistry = [
    ('KatanaPanel', 2.0, 'DxPresetBrowser', PrmanPresetBrowser),
]
