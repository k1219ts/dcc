...
class JobMain:
    ...
    def denoise_jobscript(self, parent):
        ...
        for rs in self.nodeSettings:
            ...
            folderDelTaks = author.Task(title='folderDel')
            parent.addChild(folderDelTaks)
                ...
                if ds['denoise'] == 2:
                    ...
                    for output in rs.outputs:
                        if output['name'] == 'primary':
                            ...
                            auxLocation = str(exrDir) + '/Denoise'
                            cmd = ['rm -rfv' +' '+ auxLocation + '/aux-specular'+' '+ auxLocation + '/aux-albedo'+' '+ auxLocation + '/aux-alpha'+' '+ auxLocation + '/aux-irradiance'+' '+ auxLocation + '/aux-diffuse']                        
                            folderDelTaks.addCommand(author.Command(argv=cmd))
    ...
    def frameDenoise(self, ds, frame, parent):
        ...
        if ds['denoise'] == 1:
            ...
            self.denoiseCommand(ds['primary'], frame, ds, strAov, primaryTask)
            # aovs
            for fn in ds['aovs']:
                aov = fn.split('.')[-3]
                aovTask = author.Task(title='%s %04d' % (aov, frame))
                denoiseTask.addChild(aovTask)
                self.denoiseCommand(fn, frame, ds, strAov, aovTask)
    ...
    def denoiseCommand(self, filename, frame, ds, aov, parent):
        if ds.get('version') and ds['version']:
