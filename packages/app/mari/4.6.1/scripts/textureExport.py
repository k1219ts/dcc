...
def GetVersion(dirPath, overWrite=True):
    ...
    if overWrite:
        next = 1
    if source:
        version = 'v%03d' % (int(source[-1][1:]) + next)
    else:
        version = 'v001'
    return version
...
class ExportChannel():
    ...
    def doIt( self ):
        ...
        self.Progress.close()
        if self.message:
            mari.utils.message( string.join(self.message, '\n') )
    ...
    def exportedFileRename( self, patches, channel, extension ):
        for i in patches:
            ...
            if self.TextureNameInfo[uvindex]:
                ...
                if self.TextureNameInfo.has_key('multiuv'):
                    if uvindex in self.TextureNameInfo['multiuv'].keys():
...
class ExporterDialog( QtWidgets.QDialog ):
    def __init__( self, parent=None ):
        ...
        self.setWindowTitle( 'TextureName Window' )
        ...
        if "show" in splitPath:
            ...
            if orgShowDir != showDir:  # configSetup and projectSetup equal.
                if not "_pub" in orgShowDir:  # original file convention. /show/$SHOW/asset/type/$ASSETNAME/
                    assetName = splitPath[splitPath.index("show") + 4]
                    versionDir = os.path.join(showDir, "asset", assetName)

                    if "element" in splitPath:
                        elementName = splitPath[splitPath.index("element") + 1]
                        self.ui.elementCheckBox.setChecked(True)
                        self.ui.elementEdit.setText(elementName)
                        versionDir = os.path.join(versionDir, "element", elementName)
                    versionDir = os.path.join(versionDir, "texture", "images")
                    version = GetVersion(versionDir)
        ...
        self.ui.elementLabel.setVisible(self.ui.elementCheckBox.isChecked())
        self.ui.elementEdit.setVisible(self.ui.elementCheckBox.isChecked())
        self.ui.elementCheckBox.stateChanged.connect(self.elementCheckedClick)
        self.ui.elementEdit.textChanged.connect(self.elementChange)
    ...
    def channelWidget( self ):
        ...
        for i in channels:
            ...
            exec( 'self.ui.channelLayout.addWidget( self.%s, %s, %s )' % (widgetName, index/4, index - (index/4)*4 + 1 ) )
    ...
    def elementCheckedClick(self, state):
        self.ui.elementLabel.setVisible(self.ui.elementCheckBox.isChecked())
        self.ui.elementEdit.setVisible(self.ui.elementCheckBox.isChecked())
        self.calcOutDir()
    ...
    def assetNameChange(self):
        ...
        modelVerList = glob.glob("%s/v*" % modelVerDir)
        modelVerList.sort()
        if modelVerList:
            last = os.path.basename(modelVerList[-1])
            self.ui.dataVersionEdit.setText('v%s' % str(int(last[1:])).zfill(3))
        else:
            self.ui.dataVersionEdit.setText("v001")

        textureVerList = glob.glob("%s/v*" % textureVerDir)
        textureVerList.sort()
        print textureVerList
        if textureVerList:
            last = os.path.basename(textureVerList[-1])
            print "assetName", last, int(last[1:]), str(int(last[1:]) + 1).zfill(3)
            self.ui.textureVersionEdit.setText('v%s' % str(int(last[1:]) + 1).zfill(3))
        else:
            self.ui.textureVersionEdit.setText("v001")
        self.versionChange()
        self.calcOutDir()
    ...
    def elementChange(self):
        if not self.ui.elementCheckBox.isChecked():
            return
        print "elementChange"
        ...
        if self.ui.shotCheckBox.isChecked():
            ...
            rootPath = os.path.join(showDir, "shot", seq, self.ui.shotEdit.text())
        assetName = str(self.ui.assetNameEdit.text())
        elementName = str(self.ui.elementEdit.text())
        assetPath = os.path.join(rootPath, "asset", assetName, "element", elementName)
        modelVerDir = os.path.join(assetPath, "model")
        textureVerDir = os.path.join(assetPath, "texture", "images")
        modelVerList = glob.glob("%s/v*" % modelVerDir)
        modelVerList.sort()
    ...
    def versionChange(self):
        ...
        if self.ui.elementCheckBox.isChecked():
            rootPath = os.path.join(rootPath, "element", str(self.ui.elementEdit.text()))
    ...
    def calcOutDir(self):
        ...
        if self.ui.elementCheckBox.isChecked():
            filepath = os.path.join(filepath, "element", elementName)
    ...
    def texture_export( self ):
        ...
        if not shotName:
            ...
            # if elementName:
            #     txPath = os.path.join(txPath, "element", elementName)
            ...
            if elementName:
                txPath = os.path.join(txPath, "element", elementName)
            txPath = os.path.join(txPath, "texture", "tex", textureVersion)
            # Texture.MakeTexAttr(txPath=txPath, modelVersion=dataVersion).doIt()
        self.close()
