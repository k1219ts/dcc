...
class ProjectDialog( QtWidgets.QDialog ):
    ...
    def setProject_proc( self ):
        importFormat = ['abc', 'obj','usd']
        current = self.fileModel.filePath( self.dir_treeView.currentIndex() )
        self.projGeo_lineEdit.setText( current )
        print('setProject_proc:current ')

        splitPath = current.split('/')
        path = '/'.join(splitPath[:splitPath.index('model')])
        texDir = os.path.join(path, 'texture')
        self.projRoot_lineEdit.setText(dxm.dirReMap(texDir))
