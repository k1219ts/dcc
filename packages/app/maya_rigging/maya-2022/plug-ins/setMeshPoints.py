...
from maya.api.OpenMaya import (MFnPlugin, MPxCommand, MArgParser, MSyntax,
                               MFn, MGlobal, MFnMesh, MPointArray, MColorArray)
...
class setMeshPoints(MPxCommand):
    NAME = "setMeshPoints"
    POINT_FLAG = "-p"
    ...
    def doIt(self, args):
        ...
        if not points and not colors:
            sys.stderr.write("no flag is set.")
...
def initializePlugin(mobject):
    ...
    try:
        mplugin.registerCommand(setMeshPoints.NAME, setMeshPoints, setMeshPoints.syntax_creator)
    except:
        sys.stderr.write("Failed to register {}".format(setMeshPoints.NAME))


def uninitializePlugin(mobject):
    mplugin = MFnPlugin(mobject)
    try:
        mplugin.deregisterCommand(setMeshPoints.NAME)
    except:
        sys.stderr.write("Failed to unregister {}".format(setMeshPoints.NAME))
