...
uiFile = os.path.join(os.path.dirname(currentpath), "../ui/ifConv2017.ui")
...
class Window(QtWidgets.QMainWindow):
    ...
    def connectSignal(self):
        self.itfBtn.clicked.connect(self.main)
    ...
    def IKPOVBake(self, POV_set_List):
        ...
        for i in range(len(POV_set_List)):
            ...
            cmds.connectAttr('%s:%s.parentInverseMatrix' % (nsChar[0], self.IKPOV_list[i][3]), '%s.matrixIn[1]' % multMatrix_node)
    ...
    def ikTofk(self, blendCtrl, blendList):
        ...
        cmds.bakeResults(cmds.ls(sl=True), simulation=True, t=(self.minTime, self.maxTime), sampleBy=1, dic=True, pok=True, sac=False, ral=False, bol=False, mr=True, controlPoints=False, shape=True)
    ...
    def fkToik(self, blendCtrl, povSetList):
        ...
        cmds.bakeResults(cmds.ls(sl=True), simulation=False, t=(self.minTime, self.maxTime), sampleBy=1, dic=True, pok=True, sac=False, ral=False, bol=False, mr=True, controlPoints=False, shape=True)
        cmds.delete(stemCon)
        cmds.delete(locL)
        # IK Ctrls On
        cmds.currentTime(self.minTime)
