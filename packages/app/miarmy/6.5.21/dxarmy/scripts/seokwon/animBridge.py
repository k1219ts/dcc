...
class Window():
    def __init__(self):
        ...
        self.moduleJoint_list = ['C_Skin_hip_JNT', 'C_Skin_spine1_JNT', 'C_Skin_spine2_JNT', 'C_Skin_spine3_JNT',
                                 ...
                                 'L_Skin_ball_JNT', 'R_Skin_leg_JNT', 'R_Skin_lowLeg_JNT', 'R_Skin_foot_JNT', 'R_Skin_ball_JNT']
        ...
        self.mocapJoint_list = ['Crw_Hips', 'Crw_Spine', 'Crw_Spine1', 'Crw_Spine2', 'Crw_Spine3', 'Crw_Neck', 'Crw_Head', 'Crw_LeftShoulder', 'Crw_LeftArm', 'Crw_LeftForeArm', 'Crw_LeftHand',
                                'Crw_LefthandSub1', 'Crw_RightShoulder', 'Crw_RightArm', 'Crw_RightForeArm', 'Crw_RightHand', 'Crw_RightHandSub1', 'Crw_LeftUpLeg', 'Crw_LeftLeg', 'Crw_LeftFoot',
                                'Crw_LeftToeBase', 'Crw_RightUpLeg', 'Crw_RightLeg', 'Crw_RightFoot', 'Crw_RightToeBase']
        ...
        self.IKController_list = ['root_CON', 'C_IK_lowBody_CON', 'C_IK_upBodyRot1_CON', 'C_IK_upBodyRot2_CON',
                                  ...
                                  'R_IK_foot_CON', 'L_IK_handVec_CON', 'R_IK_handVec_CON', 'L_IK_footVec_CON', 'R_IK_footVec_CON']
        ...
        if str(cmds.about(version=True)) == '2017':
    ...
    def IKPOVBake(self):
        ...
        for i in range(len(POV_set_List)):
            ...
            cmds.connectAttr('%s:%s.parentInverseMatrix' % (nsChar[0], self.IKPOV_list[i][3]),
                             '%s.matrixIn[1]' % multMatrix_node)  ## IK poleVector Bake
    ...
    def getMcp(self):
        ...
        cmds.bakeResults(cmds.ls(sl=True), hi=True, simulation=True, t=(int(self.minCurrent), int(self.maxCurrent)), sampleBy=1, dic=True,
                         pok=True, sac=False, ral=False, bol=False, mr=True, controlPoints=False, shape=True)
    ...
    def saveHikAnim(self, exAnim, restFrame, startFrame):
        ...
        cmds.file('/dexter/Cache_DATA/animation/A14_Asset/HumanIK_for_AnimBrowser/HIKJoint.ma', i=True, type='mayaAscii',
                  ra=True, mergeNamespacesOnClash=True, namespace=self.nsChar, options='v=0')
        ...
        HIKJoint_constraint_list = []
        for i in range(len(self.HIKJoint_list)):
            temp_point_con = cmds.parentConstraint("%s:%s" % (self.nsChar, self.moduleJoint_list[i]), "%s:%s" % (self.nsChar, self.HIKJoint_list[i]), mo=1, w=1)
            HIKJoint_constraint_list.append(temp_point_con[0])  # constrain 일괄 삭제를 위한 리스트 축적
        cmds.select("%s:%s" % (self.nsChar, "Hips"), hi=True)

        cmds.bakeResults(cmds.ls(sl=True), simulation=True, hi=True, t=(int(self.minCurrent), int(self.maxCurrent)), sampleBy=1,
    ...
    def loadHikAnim(self, animDir):
        ...
        cmds.select(cl=True)
        cmds.file('/dexter/Cache_DATA/animation/A14_Asset/HumanIK_for_AnimBrowser/HIKJoint.ma', i=True, type='mayaAscii', ra=True, mergeNamespacesOnClash=True, namespace='libr', options='v=0')
        cmds.select("libr:Hips")
        cmds.file(animDir, i=True, iv=True)
        curMin = cmds.playbackOptions(q=True, min=True)
        curMax = cmds.playbackOptions(q=True, max=True)
        ...
        for j in range(len(self.HIK_attatch_toIK_list)):
            self.toIK_Const = cmds.parentConstraint("%s:%s" % (self.nsChar, self.HIK_attatch_toIK_list[j]), "%s:%s" % (self.nsChar, self.IKController_list[j]), mo=1, w=1)
        ...
        if os.path.exists(jPath):
            self.neverDieMyGraph(jPath, curMin)
        else:
            pass
        if self.delConstraints == None:
            pass
        ...
        cmds.filterCurve(f="euler")
        cmds.select(cl=True)
        cmds.playbackOptions(ast=curMin, min=curMin, max=curMax)
    ...
    def impMc(self):
        ...
        cmds.file('/dexter/Cache_DATA/animation/A14_Asset/HumanIK_for_AnimBrowser/mocapJoint.ma', i=True, type='mayaAscii',
                  ra=True, mergeNamespacesOnClash=True, namespace="mcp", options='v=0')
    ...
    def neverDieMyGraph(self, jfilePath, curMin):
        ...
        fkJson = jData['bcf']
        if jData['frInfo']:
            restFr = jData['frInfo'][0]
            actFr = jData['frInfo'][1]
            del jData['frInfo']
        del jData['pl']
        del jData['nk']
        del jData['refFile']
        ...
        for w in jData.keys():
            ...
            if cmds.ls(type="transform").count(pConNm) == 1:
                    ...
                    if cmds.attributeQuery(str(e), n=pConNm, ex=True) == 1:
                        if cmds.keyframe(pConNam, at=str(e), q=True) != None:
                            for c in range(len(jData[h][e].keys())):
                                ...
                                cmds.keyTangent(pConNm, at=str(e), t=(fr,fr), l=val[c][4])
                                cmds.keyTangent(pConNm, at=str(e), t=(fr,fr), ia=val[c][0])
                                cmds.keyTangent(pConNm, at=str(e), t=(fr,fr), oa=val[c][1])
                                cmds.keyTangent(pConNm, at=str(e), t=(fr,fr), iw=val[c][2])
                                cmds.keyTangent(pConNm, at=str(e), t=(fr,fr), ow=val[c][3])
                        else:
                            pass
                    else:
                        pass
                ...
                pass
        if actFr:
            offsetFrames = int(curMin - actFr)
            cmds.select(cl=True)
            cmds.select(cmds.ls(self.nsChar + ":*_CON"))
            cmds.selectKey(cmds.ls(sl=True))
    ...
    def makeTpose(self):
        ...
        # tz 값을 어깨와 동일하게 맞추기
        plsz = cmds.getAttr(L_uA + ".tz")
        cmds.setAttr(L_hndCon + ".tz", plsz)
        Rplsz = cmds.getAttr(R_uA + ".tz")
        cmds.setAttr(R_hndCon + ".tz", Rplsz)

        # 손목 회전을 위한 각도 값 쿼리
        R_angx = cmds.getAttr(R_uA + ".tx") - cmds.getAttr(R_hNd + ".tx")
        R_angy = cmds.getAttr(R_uA + ".ty") - cmds.getAttr(R_hNd + ".ty")
        R_en = (R_angx**2 + R_angy**2)**0.5
        R_rad = math.acos(R_angx/R_en)
        R_rigAngle = R_rad*(180/math.pi)
        R_plang = cmds.getAttr(R_hndCon + ".rz") - R_rigAngle

        L_angx = cmds.getAttr(L_hNd + ".tx") - cmds.getAttr(L_uA + ".tx")
        L_angy = cmds.getAttr(L_uA + ".ty") - cmds.getAttr(L_hNd + ".ty")
        L_en = (L_angx ** 2 + L_angy ** 2) ** 0.5
        L_rad = math.acos(L_angx / L_en)
        L_rigAngle = L_rad * (180 / math.pi)
        L_plang = cmds.getAttr(L_hndCon + ".rz") + L_rigAngle

        cmds.setAttr(R_hndCon + ".rz", R_plang)
        cmds.setAttr(L_hndCon + ".rz", L_plang)

        # ty 값을 어깨와 동일하게 맞추기
        plsy = cmds.getAttr(L_hndCon + ".ty") + cmds.getAttr(L_uA + ".ty") - cmds.getAttr(L_hNd + ".ty")
        cmds.setAttr(L_hndCon + ".ty", plsy)
        Rplsy = cmds.getAttr(R_hndCon + ".ty") + cmds.getAttr(R_uA + ".ty") - cmds.getAttr(R_hNd + ".ty")
        cmds.setAttr(R_hndCon + ".ty", Rplsy)

        # 팔 길이를 계산하여 T 포즈가 되도록 손목을 x축으로 당겨주기
        stxUpArm = cmds.getAttr(L_fA + ".tx") - cmds.getAttr(L_uA + ".tx")
        styUpArm = cmds.getAttr(L_uA + ".ty") - cmds.getAttr(L_fA + ".ty")
        stzArm = cmds.getAttr(L_fA + ".tz") - cmds.getAttr(L_uA + ".tz")
        stxforeArm = cmds.getAttr(L_hNd + ".tx") - cmds.getAttr(L_fA + ".tx")
        styforeArm = cmds.getAttr(L_hNd + ".ty") - cmds.getAttr(L_fA + ".ty")
        dix = cmds.getAttr(L_hNd + ".tx") - cmds.getAttr(L_uA + ".tx")
        ...
        RstxUpArm = cmds.getAttr(R_uA + ".tx") - cmds.getAttr(R_fA + ".tx")
        RstyUpArm = cmds.getAttr(R_uA + ".ty") - cmds.getAttr(R_fA + ".ty")
        RstzArm = cmds.getAttr(R_fA + ".tz") - cmds.getAttr(R_uA + ".tz")
        RstxforeArm = cmds.getAttr(R_fA + ".tx") - cmds.getAttr(R_hNd + ".tx")
        RstyforeArm = cmds.getAttr(R_hNd + ".ty") - cmds.getAttr(R_fA + ".ty")
        Rdix = cmds.getAttr(R_uA + ".tx") - cmds.getAttr(R_hNd + ".tx")
        ...
        # 폴벡터를 팔꿈치에서 Z축으로 A 만큼 떨어지도록 만들기
        delCon = cmds.pointConstraint(L_fA, self.nsChar + ":" + "L_IK_handVec_CON")
        cmds.delete(delCon)
        polV = cmds.getAttr(self.nsChar + ":" + "L_IK_handVec_CON.tz")  # - A
        cmds.setAttr(self.nsChar + ":" + "L_IK_handVec_CON.tz", polV)

        RdelCon = cmds.pointConstraint(R_fA, self.nsChar + ":" + "R_IK_handVec_CON")
        cmds.delete(RdelCon)
        RpolV = cmds.getAttr(self.nsChar + ":" + "R_IK_handVec_CON.tz") # - A
        cmds.setAttr(self.nsChar + ":" + "R_IK_handVec_CON.tz", RpolV)
    ...
    def saveKeyFrames(self, exAnim, restFrame, startFrame):
        ...
        jset['bcf'] = dict()
        jset['frInfo'] = [restFrame, startFrame]
