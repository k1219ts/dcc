...
uiFile = os.path.join(os.path.dirname(currentpath), "ui/makeActionFilesUI.ui")

MayaVersion = str(int(mel.eval("getApplicationVersionAsFloat")))
animPlug = '/usr/autodesk/maya' + MayaVersion + '/bin/plug-ins/animImportExport.so'
...
class Window(QtWidgets.QMainWindow):
    def __init__(self, parent = None):
        ...
        self.connectSignal()
        self.num = 1
        self.movPath.setText("/dexter/Cache_DATA/CRD/Action/actionMov.mov")

    def connectSignal(self):
        self.btnSet.clicked.connect(self.sceneOpen)  # 버튼 등록 및 실행
        self.btnAct.clicked.connect(self.crtAct)
        self.btnSav.clicked.connect(self.savAct)
        self.btnTls.clicked.connect(self.timeSet)
        self.btnAnim.clicked.connect(self.nextanFile)
        self.btnPrv.clicked.connect(self.setPrv)
        self.btnHud.clicked.connect(self.hudSet)
        self.btnAce.clicked.connect(self.actionEdit)
        self.btnCrw.clicked.connect(self.openCrw)
        self.btnMov.clicked.connect(self.brwMov)
        self.btnPb.clicked.connect(self.playBlst)
        self.revBtn.clicked.connect(self.animReverse)
    ...
    def animReverse(self):
        ...
        # 각 프레임에서의 조인트 Rotation 값 쿼리, Translate 값이 있는 Hip 조인트만 쿼리
        jDct = dict()
        for j in jointList:
            jDct[j] = dict()
            for k in rAtts:
    ...
    def nextanFile(self):
        ...
        self.lineEditA.setText(self.showAn)

    def resetSc(self):
        hudList = ["CRD_Frame", "CRD_Duration", "CRD_Action", "CRD_Path", "CRD_Type", "CRD_Blend_Cycle", "CRD_Blend_Entry", "CRD_Blend_Exit"]
        for i in hudList:
    ...
    def crtAct(self):
        McdPlusPack.McdCreateActionCmd(self.showAn)
    ...
    def savAct(self):
        ...
        McdPlusPack.McdSaveAction(self.animDir[0])  # Save Action
        ...
        self.movPath.setText(nName)
        cmds.select(fcdAction)

    def openCrw(self):

        pbDir = os.sep.join(self.movPath.text().split(os.sep)[:-1])
    ...
    def setPrv(self):
        ...
        if cmds.nodeType(self.sel) == "McdAction":
            ...
            if self.typeDic.has_key(tuple(statList)):
                ...
                cmds.setAttr(self.miarmy + ".txState", 1)
                cmds.setAttr(self.miarmy + ".tyState", 0)
                cmds.setAttr(self.miarmy + ".tzState", 1)
                cmds.setAttr(self.miarmy + ".rxState", 0)
                cmds.setAttr(self.miarmy + ".ryState", 0)
                cmds.setAttr(self.miarmy + ".rzState", 0)
                self.type_line = "Locomotion"
    ...
    def playBlst(self):
        pbDir = os.sep.join(self.movPath.text().split(os.sep)[:-1])
        pbFile = self.movPath.text().split(os.sep)[-1].split(".")[0]
        pbPath = pbDir + "/" + pbFile
