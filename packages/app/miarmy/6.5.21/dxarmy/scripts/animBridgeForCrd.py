...
if cmds.pluginInfo(animPlug, q=True, l=True) == False:
    try:
        ...
        cmds.error("Anim Import & Export Plug-ins Load Error.")

mel.eval("cycleCheck -e off")
currentpath = os.path.abspath(__file__)
currentDir = os.path.dirname(currentpath)
uiFile = os.path.join(os.path.dirname(currentpath), "ui/animBridgeForCrdUI.ui")
...
class Window(QtWidgets.QMainWindow):
    ...
    def expAnim(self):
        ...
        cmds.select(cl=True)
        cmds.select("%s:%s" % (self.nsChar, "Crw_Hips"), hi=True)
        exAnim = cmds.fileDialog2(startingDirectory="/", fileMode=0, fileFilter="animExport (*.anim)", caption="Export Anim")
    ...
    def impAnim(self):
        ...
        cmds.file('%s/JointSetForRetargeting/huType_hikJoint.ma' % currentDir, i=True,
        ...
        cmds.file('%s/JointSetForRetargeting/HIKJoint.ma' % currentDir, i=True,
