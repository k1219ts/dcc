...
1. if not nodes,
    find node by 'XG_ExportSet' objectSet

Script Command
> dxsUsd.XgenAssetExport(showDir=$showDir, asset=$assetName, version=$ver).doIt()
'''
...
def GetXGenBoundMesh(nodes=None):
    boundMeshes = list()
    if nodes:
        for node in nodes:
            for n in cmds.listHistory(node):
                if cmds.nodeType(n) == 'xgmSplineBase':
                    shapes = cmds.listConnections(n, s=True, sh=True, type='surfaceShape')
                    if shapes:
                        boundMeshes += cmds.ls(shapes, l=True)
    else:
        for n in cmds.ls(type='xgmSplineBase'):
            shapes = cmds.listConnections(n, s=True, sh=True, type='surfaceShape')
            if shapes:
                boundMeshes += cmds.ls(shapes, l=True)

    return list(set(boundMeshes))


class XGenScene:
    def __init__(self, filename, open=False, show='', shot='', user=''):
        cmds.file(new=True, f=True)
        dxsMayaUtils.PluginSetup(['backstageMenu', 'pxrUsd', 'xgenToolkit', 'DXS_XGenPlugin'])
        ...
        # initialize
        if not cmds.ls('XG_ExportSet', r=True):
            MsgSender.sendMsg("# msg: Not found 'XG_ExportSet' - %s" % filename, show, shot, user)

        # Member Variables
        self.xgenExportNodes = list()
        self.xgenImportNodeMap = dict()
        self.xgenBoundMeshes = list()

        for s in cmds.ls('XG_ExportSet', r=True):
            self.xgenExportNodes += cmds.sets(s, q=True)

        if not self.xgenExportNodes:
            return

        self.initialize()
        self.getXgenBoundMesh()
    ...
    def getXgenBoundMesh(self):
        self.xgenBoundMeshes = GetXGenBoundMesh()
...
class GetExportData:
    ...
    def getMeshAttributes(self, node):
        splineBaseNodes = list()
        for xgenNode in cmds.listHistory(node, bf=True):
            if cmds.nodeType(xgenNode) == 'xgmSplineBase':
                splineBaseNodes.append(xgenNode)
        if not splineBaseNodes:
            return

        result = dict()
        print splineBaseNodes
        inBodyMesh = cmds.listConnections('%s.boundMesh' % splineBaseNodes[0], sh=True)[0]
        ...
        if not result.has_key('modelVersion'):
            _msg = "[XGen.GetExportData] - Not found 'modelVersion' attribute -> %s" % inBodyMesh
...
class XGenGeomExport(Arguments.CommonArgs):
    '''
    Args:
    ...
    def warningCheck(self):
        if not self.nodes:
            if not cmds.objExists('XG_ExportSet'):
                dxsMsg.Print('warning', "[XGen.XGenGeomExport] - not found 'XG_ExportSet'")
                return False
            self.nodes = cmds.sets('XG_ExportSet', q=True)
        if not self.nodes:
            dxsMsg.Print('warning', "[XGen.XGenGeomExport] - not found 'XGEN' nodes")
            return False
        return True

    def doIt(self, postProcess=1):
        '''
        XGen geom data export
        ...
        if not self.warningCheck():
            return
        dxsMsg.Print('warning', "[XGen.XGenGeomExport Nodes] -> %s" % string.join(self.nodes, ', '))
        ...
        print jobCmd, self.fr[0], self.fr[1]
        cmds.DX_XgenUsdCmd(j=jobCmd, startFrame=self.fr[0], endFrame=self.fr[1], step=self.step, v=True)
        ...
        if postProcess == 2:
            ...
            geomFile = '{DIR}/groom_deforms.usd'.format(DIR=self.outDir)
            args = {'SdfPath': '/groom/xgen', 'comment': self.comment}
            if self.customData:
    ...
    def outDataCollapsed(self, outData):    # outData: GetExportData
        ...
        highgeom = '{DIR}/groom_deforms.high_geom.usd'.format(DIR=self.outDir)
        SessionUtils.MakeReferenceStage(highgeom, highRefs, SdfPath='/groom', addChild=True, comment=self.comment)
        UpdateAttributes(highgeom, outData, self.customData)
        lowgeom  = '{DIR}/groom_deforms.low_geom.usd'.format(DIR=self.outDir)
        SessionUtils.MakeReferenceStage(lowgeom, lowRefs, SdfPath='/groom', addChild=True, comment=self.comment)
        UpdateAttributes(lowgeom, outData, self.customData)
        return highgeom, lowgeom


class UpdateAttributes:
    def __init__(self, geomfile, outData, customData, prefixPath='/groom'):
    ...
    def parsePath(self):
        ...
        if self.customData and self.customData.has_key('layer'):
            if self.customData['layer'].has_key('xgenAssetFile'):
                xgenAssetFile = self.customData['layer']['xgenAssetFile']
                splitPath = xgenAssetFile.split('/')
...
#
#-------------------------------------------------------------------------------
class XGenAssetExport(Arguments.AssetArgs):
    '''
    ZENN assert export
    '''
    def __init__(self, nodes=list(), **kwargs):
        dxsMayaUtils.PluginSetup(['backstageMenu', 'pxrUsd', 'xgenToolkit'])
        if not cmds.pluginInfo('DXS_XGenPlugin', q=True, l=True):
            MsgSender.sendMsg('# msg : DXS_XGenPlugin plugin setup error!')

        self.nodes = nodes
        Arguments.AssetArgs.__init__(self, **kwargs)
        if not self.outDir and self.assetDir:
            self.outDir = self.assetDir + '/groom'
        self.computeVersion()
    ...
    def warningCheck(self):
        if not self.nodes:
            if not cmds.objExists('XG_ExportSet'):
                dxsMsg.Print('warning', "[XGen.XGenAssetExport] - not found 'XG_ExportSet'")
                return False
            self.nodes = cmds.sets('XG_ExportSet', q=True)
        if not self.nodes:
            dxsMsg.Print('warning', "[XGen.XGenAssetExport] - not found 'XGen' nodes")
            return False
        return True
    ...
    def doIt(self):
        ...
        geomFile= XGenGeomExport(geomDir, nodes=self.nodes, fr=(ctime, ctime), comment=self.comment).doIt()
        geomPayloadFile = geomFile.replace('.usd', '.payload.usd')
        SessionUtils.MakeReferenceStage(geomPayloadFile, [(geomFile, None)], SdfPath='/%s{groomVersion=%s}' % (self.assetName, self.version))
        self.makePackage(geomPayloadFile)

        self.xgenAssetSetup()

    def makePackage(self, sourceFile):
        taskFile = '{DIR}/groom.usd'.format(DIR=self.outDir)
    ...
    def xgenAssetSetup(self):
        ...
        self.xgenPubSceneFile = os.path.join(self.outDir, "scenes",
                                "{ASSET}_groom_{VER}.mb".format(ASSET=self.assetName, VER=self.version))
        # cmds.file(rename=filename)
        cmds.file(save=True)
        orgFileName = cmds.file(q=True, sn=True)
        cmd = "cp -rf %s %s" % (orgFileName, self.xgenPubSceneFile)
        os.system(cmd)

        # insert 190614, if save maya scene file, write json file from ZN_ExportSet
        print "# Debug : XGEN Nodes", self.nodes
        exportSetJsonData = {"XG_ExportSet":self.nodes}
        jsonfilename = self.xgenPubSceneFile.replace(".mb", ".json")
        ...
        if not data.has_key('groom'):
            data['groom'] = {}
        if not data['groom'].has_key(self.assetName):
            data['groom'][self.assetName] = {}

        task = "modelGroom"
        if "groomSim" in self.mayafile:
            task = "rigGroom"

        if not data['groom'][self.assetName].has_key(task):
            data['groom'][self.assetName][task] = {}
        checkKey = "{ASSET}_groom_{VER}".format(ASSET = self.assetName, VER = self.version)
        if not data['groom'][self.assetName][task].has_key(checkKey):
            data['groom'][self.assetName][task][checkKey] = {}

        data['groom'][self.assetName][task][checkKey]["assetFile"] = self.xgenPubSceneFile
        if not data['groom'][self.assetName][task][checkKey].has_key("rigVersion"):
            data['groom'][self.assetName][task][checkKey]['rigVersion'] = []
        if rigVersion and not rigVersion in data['groom'][self.assetName][task][checkKey]["rigVersion"]:
            data['groom'][self.assetName][task][checkKey]["rigVersion"].append(rigVersion)

        if task == "rigGroom":
            data['groom'][self.assetName][task][checkKey]["shotList"] = []
