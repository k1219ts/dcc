...
from ui.spanner2_ui import Ui_Form
...
class MayaActions(object):
    ...
    def getNamespace(self):
        if self.filePath.split('/')[3] == 'asset' in self.filePath:
    ...
    def importAct(self):
        ...
        if os.path.splitext(self.fileName)[-1] == ".asb":
            inc_tool_by_user.run('action.Spanner2.import_asb', getpass.getuser())
            openfile = os.path.join(self.filePath, self.fileName)
            asbClass = sgAssembly.AssemblyImport(openfile)
            asbClass.doIt()
...
class SaveDevForm(QtWidgets.QDialog):
    ...
    def getDevInfo(self):
        ...
        if src[src.index('show')+2] == 'shot':
            if src[src.index('shot')+2] in self.fileName:
                self.assetShot = 'shot'
            else:
        ...
        if self.assetShot == 'shot':
            self.shotType = temp[0]
            self.shotName = temp[1]
            self.shotWorkCode = temp[2]
            self.version = int(names.split('v')[-1].split('_')[0])#version
            
            dirList = []
            for i in os.listdir(self.filePath):
                if '_v' + str(self.version).zfill(2) in i and i.split('.')[-1] == 'mb' and len(i.split('_')) > 4:
                    dirList.append(i)
            
            if self.fileName.find('_w') > -1:
                lastWipFile = sorted(dirList)[-1].split('.')[0]
                self.wipVersion = int(lastWipFile.split('w')[-1].split('_')[0])#wipversion           
                if len(temp) == 6:
                    self.dsc = temp[5]
            else:
                if dirList:
    ...
    def setDevInfo(self):
        ...
        if not self.dsc == '':
            if self.assetShot == 'shot':
                self.fileName = '%s_%s_%s_v%s_w%s_%s.mb'%(
                    self.shotType, self.shotName, self.shotWorkCode, str(self.upVersion).zfill(2),
            ...
            if self.assetShot == 'shot':
                self.fileName = '%s_%s_%s_v%s_w%s.mb'%(
                    self.shotType, self.shotName, self.shotWorkCode, str(self.upVersion).zfill(2),
...
class SavePubForm(QtWidgets.QDialog):
    def __init__(self, parent=None, fileName='', filePath=''):
        ...
        self.ui.setupUi(self)
        self.ui.addInventory_checkBox.setIcon(
            QtGui.QIcon(QtGui.QPixmap(os.path.join(CURRENTPATH, "resource/inventory.png"))))
        self.setWindowTitle('save Publish')
        self.getPubInfo()
        self.setDevInfo()
        self.setDescription()
        self.workCode = self.filePath.split('/')[6]
        if self.workCode == 'model':
            self.ui.model_groupBox.setEnabled(1)
        else:
    ...
    def getPubInfo(self):
        ...
        if self.assetShot == 'shot':
            ...
            self.version = int(names.split('v')[-1].split('_')[0])#version
            if self.fileName.find('_w') > -1:
                self.wipVersion = int(names.split('w')[-1].split('_')[0])#wipversion
                if len(temp) == 6:
                    self.description = temp[5]
            else:
                if len(temp) == 5:
    ...
    def setPubInfo(self):
        ...
        if self.assetShot == 'shot':
            if self.description:
                self.fileName = '%s_%s_%s_v%s_%s.mb' % (
                    self.shotType, self.shotName, self.shotWorkCode, str(self.version).zfill(2), self.description)
            else:
                self.fileName = '%s_%s_%s_v%s.mb' % (
                    self.shotType, self.shotName, self.shotWorkCode, str(self.version).zfill(2))
    ...
    def setDevInfo(self):
        ...
        if not self.description == '':
            if self.assetShot == 'shot':
                self.nextDev = '%s_%s_%s_v%s_w%s_%s.mb' % (
                    self.shotType, self.shotName, self.shotWorkCode, str(self.version).zfill(2),
        ...
        # description not exists:
            ...
            if self.assetShot == 'shot':
                self.nextDev = '%s_%s_%s_v%s_w%s.mb' % (
                    self.shotType, self.shotName, self.shotWorkCode, str(self.version).zfill(2),
    ...
    def savePub(self):
        ...
        # pub for model
            ...
            # from dexcmd module
            ex = None; #ExportMesh(self.savePubPath, cmds.ls(sl=True, l=True))
...
class WaringDialog(QtWidgets.QDialog):
    def __init__(self, parent=None, text=''):
        QtWidgets.QDialog.__init__(self)
        # ui
        self.label = QtWidgets.QLabel("File exists. Are you sure to save?\n")
        if text:
