...
from CrowdDialogUI import Ui_Form
...
class CrowdImportDialog(QtWidgets.QDialog):
    ...
    def initializeDataSet(self, startIndex):
        # tag 1 tier setting
        self.tagData = self.dbPlugin.getTagData("Crowd")
        self.tagDict = {}
    ...
    def loadAnimFile(self):
        titleCaption = "Load ma File"
        exrCaption = "Maya Ascii (*.ma)"
        ...
        movFile = fileName.replace('.ma', '.mov')
    ...
    def loadMovFile(self):
        ...
        maFile = fileName.replace('.mov', '.ma')
        if os.path.exists(maFile) and self.ui.animPathEdit.text() == "":
            self.ui.animPathEdit.setText(maFile)
    ...
    def okBtnClick(self):
        ...
        if msg == "OK":
            if self.parent.importCallDialogCrowd() == True:
                self.closeBtnClick()
            else:
