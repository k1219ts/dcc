...
baseDirPath = '/stdrepo/CSP/kwantae.kim/animBrowser/AnimBrowserRepository'
retargetPath = '/stdrepo/CSP/kwantae.kim/animBrowser/AnimBrowserRetargetting'

sys.path.append("/backstage/dcc/packages/ext/zelos/py")
sys.path.append("/backstage/dcc/packages/ext/zelos/lib")
...
from Item import DirItem as di
reload(di)
...
class MainForm(QtWidgets.QWidget):
    def __init__(self, parent=None):
        ...
        # Connect Setting
        self.ui.aniTreeWidget.itemClicked.connect(self.aniFileClicked)
        ...
        self.ui.aniCheckBox.stateChanged.connect(self.reCalcTag)
        self.ui.mcpCheckBox.stateChanged.connect(self.reCalcTag)
        self.ui.crdCheckBox.stateChanged.connect(self.reCalcTag)

        self.tagList = self.calcTagList()

        # Testing
        self.model = QtGui.QStringListModel()
        self.model.setStringList(self.tagList)

        tagSearchCompleter = CustomCompleter(self)
        tagSearchCompleter.setModel(self.model)
        tagSearchCompleter.setWrapAround(False)
        tagSearchCompleter.setCaseSensitivity(QtCore.Qt.CaseInsensitive)
        font = QtGui.QFont()
        font.setPointSize(13)
        tagSearchCompleter.popup().setFont(font)
        self.ui.tagSearchEdit.setCompleter(tagSearchCompleter)

        mel.eval('help -popupMode true;')
    ...
    def reCalcTag(self):
        ...
        if self.ui.aniCheckBox.isChecked():
            self.tagList += self.calcTagList("Animation")
        if self.ui.mcpCheckBox.isChecked():
            self.tagList += self.calcTagList("Mocap")
        if self.ui.crdCheckBox.isChecked():
            self.tagList += self.calcTagList("Crowd")

        self.model.setStringList(self.tagList)


    def calcTagList(self, category = ""):
        tagList = []
        for tag in self.dbPlugin.getHashTags(category):
            if tag:
                tagList.append("#" + tag)

        tagList += self.tagPlugin.getTier()
    ...
    def initializeDataSet(self):
        ...
        for tag1Tier in self.tagData:
            ...
            if not tag1Tier.has_key("tag_tier3"):
                ...
                self.tagDict[teamCategory][tag1Tier["tag_tier1"]][tag1Tier["tag_tier2"]].insert(0, tag1Tier["tag_tier3"])
        ...
        for key in self.tagDict.keys():
            ...
            for key2 in sorted(self.tagDict[key].keys()):
                ...
                for key3 in sorted(self.tagDict[key][key2]):
                    ...
                    for key4 in sorted(self.tagDict[key][key2][key3]):
                        parentText = key
    ...
    def contentTabRmbClicked(self, pos):
        ...
        # JM
        icon = Qt.QtGui.QIcon( os.path.join( str(CURRENTDIR), "retargetting/spider_purple.png") )
        menu.addAction(icon, "Import BVH Source to Joint", self.importRetargetSourceToJoint)
        menu.addAction(icon, "[under deprecate]", None)
    ...
    def modifyHashTag(self):
        ...
        if msg == 1:
            ...
            dbItem = dbConfig.updateHashTag(self.ui.ContentTabWidget.currentWidget().currentItem().contentInfo['_id'], hashTag)
            self.ui.ContentTabWidget.currentWidget().currentItem().contentInfo = dbItem

    def changeTier(self):
        dialog = ChangeTierDialog(self, self.ui.ContentTabWidget.currentWidget().currentItem().contentInfo, self.tagDict)
        dialog.exec_()
    ...
    def exportBVH(self):
        ...
        :return: X
        '''
        exportFilePath = os.path.join(retargetPath, getpass.getuser(), "exported.bvh")
        ...
        exporter.generate_joint_data( rootJnt, exportFilePath)
        self.createPreviewNode(exportFilePath)
    ...
    def importRetargetSourceToJoint(self):
        ...
        if selection:
            rootJNT = selection[0]
            if not "root_JNT" in rootJNT:
                cmds.confirmDialog(title="Error!",
                                   message="select root_JNT",
                                   icon="warning",
                                   button=["OK"])
                return

            nameSpace = ":".join(rootJNT.split(':')[0:-1])
            retargetfile = self.doRetargetting(rootJNT, nameSpace, askRemap = True)
            if retargetfile:
                if os.path.exists(retargetfile):
                    importer = bvhImporter.BVHImporter()
                    importer.isCON = False
                    if nameSpace:
                        importer.target_ns = nameSpace
    ...
    '''
    cleanup data to upper
    '''
    def doRetargetting(self, rootJointNode = None, targetNameSpace = "", askRemap = False):
        if not rootJointNode:
            cmds.confirmDialog(title="Error!",
                               message="select root_JNT",
                               icon="warning",
                               button=["OK"])
        ...
        targetfile = self.targetfile # temp target file.
        ...
        ### REMAP DIALOG ###
        if askRemap:
            remap = self.RemapConfirmDialog()  # do you remap by joint?
            if remap == 2: # YES
                self.remapDialog = RemapDialog(self, sourcefile=sourcefile, target_ns = targetNameSpace )
                ...
                if result:
                    ...
                    targetfile = self.remapDialog.exportRemapJoints()
                else:
                    return
            elif remap == 1:
                # NO ROLE
                # without REMAP
                exporter = bvhExporter.BVHExporter()
                exporter.generate_joint_data( rootJointNode, targetfile, 0, 2 )
            else:
                # CANCEL ROLE
                return
        else:
    ...
    def importRetargetSourceToCON(self):
        ...
        if selection:
            ...
            cmds.delete(cmds.ls(['proxy*_CON','proxy*_NUL'])+cmds.ls('proxy*:*',type='joint')+cmds.ls('ZArachneCharacter*'))

            # GET SCALE AND SET TO 1
            initscale = None
            initscaleLock = None
            globalscale = None
            globalscaleLock = None
    ...
    def createPreviewNode(self, filename=""):
        '''
        import BVH To ZArachneCharacter
        :param filename: bvh filepath
        :return: X
        '''
        if not cmds.pluginInfo( "ZArachneForMaya", q=True, l=True ):
            cmds.loadPlugin( "ZArachneForMaya" )
    ...
    def importCallDialog(self):
        try:
            ...
            if self.importDialog.ui.animPathEdit.text() == "":
                ...
                jsonFilePath = "{0}/{1}/{2}/{3}/{4}/{5}/{2}_{3}_{4}.json".format(baseDirPath, category, title1Tier, title2Tier, title3Tier, fileNum)
    ...
    def importCallDialogNoHIK(self):
        try:
            ...
            outputPath = "{0}/{1}/{2}/{3}/{4}/{5}/{2}_{3}_{4}.anim".format(baseDirPath, category, title1Tier, title2Tier, title3Tier, fileNum)
    ...
    def publish(self, category, tag1TierTitle, tag2TierTitle, tag3TierTitle, animFilePath, previewFilePath, isHIK = 2, jsonFilePath = "", isTractor = True, hashTag = [], movFile = "", bvhFilePath=""):
        '''
        ...
        pubBaseDirPath = "/assetlib/anim_browser/{0}/{1}/{2}/{3}/{4}".format(category, tag1TierTitle, tag2TierTitle, tag3TierTitle, fileNum)
        ...
        tractor.createJob(jobName = "{0}_{1}_{2}_{3}_{4}".format(category, tag1TierTitle, tag2TierTitle, tag3TierTitle, fileNum))
    ...
    def aniFileClicked(self, item, column):
    ...
    def tagSearchPressed(self):
        ...
        appendStr = ""
        if self.ui.aniCheckBox.isChecked():
            findCategory.append("Animation")
            appendStr += "A"
        if self.ui.mcpCheckBox.isChecked():
            findCategory.append("Mocap")
            appendStr += "M"
        if self.ui.crdCheckBox.isChecked():
            findCategory.append("Crowd")
            appendStr += "C"
