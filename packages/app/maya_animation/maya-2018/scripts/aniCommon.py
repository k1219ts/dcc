...
def undo(func):
    ...
    def _undofunc(*args, **kwargs):
        ...
        try:
            ...
            # if cmds.undoInfo(q=True, undoName=True) == func.__name__:
            #     cmds.undo()
            # raise

    return _undofunc  # lambda *args, **kwargs: mutils.executeDeferred(_undofunc, *args, **kwargs)
...
def getListDirs(path, isFile=False, fileType=None):
    ...
    for F in fileList:
        if not isFile:
            if os.path.isdir(path + os.sep + F) and not "_pub" in F:
                finalList.append(F)
        else:
...
def exportRetime2Nuke(ip, filename):
    ...
    filename = string.join([filename, filename.split(os.sep)[-1]], os.sep)
    ...
    if not f.closed:
        ...
        for imageplane in ip:
            ...
            for frame in range(min, max + 1):
                ...
                frame = cmds.getAttr('time1.outTime', time=frame)
                retimedFrame = cmds.getAttr(str(imageplane) + ".frameExtension",
                                            time=frame)
                if isinstance(retimedFrame, list):
                    f.write(" {0}".format(retimedFrame[0]))
                else:
                    f.write(" {0}".format(retimedFrame))
            f.write("}}\n")
            f.write(" filter none\n")
            f.write(" name {0}_timeWarp\n".format(imageplane))
            f.write("}\n\n")
...
def renameGcdSpiderTemp():
    sel = cmds.ls(sl=True)
    for dx in sel:
        RN_node = cmds.referenceQuery(dx, referenceNode=True)
        cmds.lockNode(RN_node, l=False)
        refFileFullName = cmds.referenceQuery(RN_node, filename=1)
        newNameSpace = cmds.file(refFileFullName, q=True, namespace=True)
        cmds.file(refFileFullName, e=True, namespace=newNameSpace)
        rnName = cmds.rename(RN_node, newNameSpace + "RN")
        cmds.lockNode(rnName, l=True)
...
def getTimeWarpScale(timewarpNode):
    keyValue = cmds.keyframe(timewarpNode, q=1, vc=1)
    keyTime = cmds.keyframe(timewarpNode, q=1)
...
def exportTimewarpToNuke(timewarp, filename):
    min = int(float(cmds.playbackOptions(q=1, min=1)))
    max = int(float(cmds.playbackOptions(q=1, max=1)))
    ...
    if not f.closed:
        ...
        for panName in cmds.getPanel(all=True):
            if 'modelPanel' in panName:
                cmds.isolateSelect(panName, state=1)
        for node in timewarp:
            f.write("TimeWarp {\n")
            f.write(" lookup {{curve x%i" % min)
            for frame in range(min, max + 1):
                retimedFrame = cmds.getAttr(node + ".output",
                                            time=frame)
                f.write(" %f" % retimedFrame)
            f.write("}}\n")
            f.write(" filter none\n")
