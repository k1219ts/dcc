...
uiFile = os.path.join(os.path.dirname(currentpath), "../ui/simpleConst.ui")
...
class Window(QtWidgets.QMainWindow):
    ...
    def connectSignal(self):
        ...
        self.helpBtn.setIcon(QtGui.QIcon(QtGui.QPixmap("/dexter/Cache_DATA/animation/A0_Artist/Choi_SeokWon/icons/system-help.png")))
        self.helpBtn.setIconSize(QtCore.QSize(24, 24))

    def openPDF(self):
        exp = "/usr/bin/evince"
        fileName = "/dexter/Cache_DATA/animation/A0_Artist/Choi_SeokWon/MyManual/selectionSet.pdf"
        subprocess.Popen([exp, fileName])

    def moveSel(self):
        try:
            cmds.manipMoveContext('Move', edit=1, mode=2)
            selns = str(cmds.ls(sl=True)[0]).split(":")[0]

            if len(cmds.ls(selns + ":root_CON")) != 0:
            ...
            elif len(cmds.ls(selns + ":C_IK_root_CON")) != 0:
