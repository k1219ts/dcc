...
class Ripple(OpenMayaMPx.MPxDeformerNode):
    ...
    def deform(self, dataBlock, geoIterator, matrix, geometryIndex):
        ...
        ampitudeValue = dataHandleAmplitude.asFloat()
        ...
        while not geoIterator.isDone():
            ...
            pointPosition.x = pointPosition.x + math.sin(geoIterator.index() + displaceValue) * ampitudeValue * \
                                                mFloatVectorArray_normal[geoIterator.index()].x * weight * envelopeValue
            pointPosition.y = pointPosition.y + math.sin(geoIterator.index() + displaceValue) * ampitudeValue * \
                                                mFloatVectorArray_normal[geoIterator.index()].y * weight * envelopeValue
            pointPosition.z = pointPosition.z + math.sin(geoIterator.index() + displaceValue) * ampitudeValue * \
