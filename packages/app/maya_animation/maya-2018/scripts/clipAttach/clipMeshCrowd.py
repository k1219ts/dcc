...
def makeUI():
    ...
    cmds.button("attachMesh", w=300, h=30, l='attach mesh', command=attachMesh)

    cmds.separator( height=40, style='in')
    cmds.button("getEntitys", w=300, h=30, l='Select Entity', command=getTargetEntityList)
    cmds.button("substituteEntity", w=300, h=30, l='Substiute mesh', command=substituteEntity)
    ...
    cmds.separator( height=40, style='in' )
    timeScaleMethod_checkBox = cmds.checkBoxGrp( numberOfCheckBoxes=2, label='Apply method', labelArray2=['Random', 'Velocity'],on1=disableVelocityUI, on2=disableRandomTimescaleUI,v1=1 )
...
def getTargetEntityList(*argv):
    ...
    targetEntityList = cmds.ls(sl=1)

    for targetEntity in targetEntityList:
        existUsdAtt = cmds.attributeQuery('usdVariantSet_clip',node=targetEntity,ex=1)
        if(existUsdAtt == 0):
            cmds.error("선택한 오브젝트중 USD클립이 아닌 오브젝트가 포함되어 있습니다.")
...
def substituteEntity(*argv):
    ...
    for i in range(len(targetEntityList)):
        ...
        if(cmds.objExists(clipMesh[sourceNum])):
            ...
            cmds.setAttr(substituteClipMesh+".usdVariantSet_clip",entityClip, type='string')
...
def applyOffset(*argv):
    ...
    for entity in entityList:
        existUsdAtt = cmds.attributeQuery('usdVariantSet_clip',node=entity,ex=1)
        if(existUsdAtt == 0):
            cmds.error("선택한 오브젝트중 USD클립이 아닌 오브젝트가 포함되어 있습니다.")
    ...
    for entity in entityList:
        ...
        entityTimeConnect=cmds.connectionInfo(entity+'.time',sfd=1)
