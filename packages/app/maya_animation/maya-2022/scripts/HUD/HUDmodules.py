...
def getCamera():
    ...
    panelName = cmds.getPanel(wf=True)
    if not panelName in modelPanels:
        cmds.confirmDialog(title='Warning', message='Focus On CameraView',
                           button='close', cancelButton='close',
                           dismissString='close')
        return None
    camera = cmds.modelPanel(panelName, q=True, camera=True)
...
def mg_CreateHUD(artistName, sceneName, status, progress, fontSize="large"):
    ...
    cmds.headsUpDisplay('sceneName', l="Scene Info   ", allowOverlap=True,
                        event="SceneOpened",
                        b=1,
                        s=3,
                        lfs=fontSize,
    ...
    if status:
        cmds.headsUpDisplay('status', l="Status        ", allowOverlap=1,
                            ...
                            dataFontSize=fontSize,
                            command=("'%s'" % (status)))
    cmds.headsUpDisplay('camName', allowOverlap=1,
                        b=0,
                        s=8,
                        lfs=fontSize,
                        bs="small",
