...
# 모듈이름 수정해야함
sys.path.append("/stdrepo/ANI/Application/Maya/Script/MayaScript/retargeter/BipedRetarget")
import BipedRetarget_edit
reload(BipedRetarget_edit)
...
class Retarget:
    ...
    initPoseCtrl_list = ['L_IK_foreLeg_CON', 'R_IK_foreLeg_CON', 'L_IK_hindLeg_CON', 'R_IK_hindLeg_CON','L_wistRoll_CON','R_wistRoll_CON','L_ankleRoll_CON','R_ankleRoll_CON',
                         ...
                         'C_IK_upBody_CON', 'C_IK_spineMiddle_CON','C_IK_lowBody_CON','C_IK_spineFront_CON','C_IK_spineBack_CON','C_IK_upBodySub_CON','C_IK_spineFront_CON','C_IK_spineBack_CON','C_IK_lowBodySub_CON',
                         'C_IK_root_CON','C_IK_hip_CON','L_IK_hip_CON','R_IK_hip_CON',
                         'L_IK_clavicle_world_CON','R_IK_clavicle_world_CON',
                         'C_IK_head_CON', 'C_IK_headSub_CON','C_IK_neck1_CON', 'C_IK_neckMiddle_CON','C_IK_neckFront_CON','C_IK_neckBack_CON','C_IK_neck1Sub_CON',
    ...
    upBodyCtrls = ['C_IK_upBody_CON', 'C_IK_upBodySub_CON','C_IK_spineFront_CON']
    middleBodyCtrls = ['C_IK_spineMiddle_CON']
    lowBodyCtrls = ['C_IK_lowBody_CON', 'C_IK_spineBack_CON','C_IK_lowBodySub_CON']
...
def exeRetargetClass(inst):
    ...
    #keyPst = findMinMaxKey(mocapCtrls, nameSpace)
...
def retarget_bake_EXE(*argv):


    selCtrl = cmds.ls(sl=1)

    
    if(cmds.namespace(ex="E9dog_tem")!=1):
        cmds.file('/stdrepo/ANI/Asset/Rig/Project_rigs/creature/dog/slc_dog/forMCP/E9_rig_forKeySwitch_v001.mb', r=True, type='mayaBinary',  mergeNamespacesOnClash=True, namespace="E9dog_tem", options='v=0')

    else:
        cmds.error("already exists E9 template rig")


    ctrlList = ['C_IK_head_CON', 'C_IK_neckMiddle_CON', 'C_IK_neck1_CON', 'L_IK_clavicle_world_CON', 'L_IK_clavicle_CON', 'R_IK_clavicle_world_CON', 'R_IK_clavicle_CON', 'C_IK_upBody_CON', 'L_IK_scapula_CON', 'R_IK_scapula_CON', 'C_IK_spineMiddle_CON', 'C_IK_lowBody_CON', 'C_IK_hip_CON', 'L_IK_hip_CON', 'R_IK_hip_CON', 'tail1_CON', 'tail2_CON', 'tail3_CON', 'tail4_CON', 'L_IK_edbow_CON', 'R_IK_edbow_CON', 'L_IK_knee_CON', 'R_IK_knee_CON', 'C_IK_root_CON', 'L_wistRoll_CON', 'R_wistRoll_CON', 'L_ankleRoll_CON', 'R_ankleRoll_CON', 'R_IK_foreLeg_CON', 'L_IK_foreLeg_CON', 'L_IK_hindLeg_CON', 'R_IK_hindLeg_CON', 'move_CON', 'direction_CON', 'place_CON']

    nameSpace = getNamespace(selCtrl)

    cmds.select(cl=1)



    #캐릭터의 전체키를 템플릿 캐릭터로 복사
    for ctrl in ctrlList:
        #컨트롤러에 키가 존재하는지 확인;
        if(cmds.objExists(nameSpace + ":" + ctrl)):
            if (cmds.keyframe(nameSpace + ":" + ctrl, tc=1, q=1) != None):
                cmds.copyKey(nameSpace + ":" + ctrl)
                cmds.pasteKey("E9dog_tem:" + ctrl)


    cmds.select("E9dog_tem:C_IK_head_CON", r=1)
    abs_local()


    cmds.select("E9dog_tem:C_IK_head_CON", r=1)
    cmds.select(nameSpace + ":C_IK_head_CON", add=1)
        ...
        for controler in Retarget.allControler:
            ...
            if (cmds.objExists(source)):
                ...
                if (cmds.keyframe(source, tc=1, q=1) != None):
                    cmds.copyKey(source)
                    if(cmds.objExists(target)):
                        cmds.pasteKey(target, o='replace')
    ...
    if(cmds.namespace(ex="E9dog_tem")==1):
        cmds.file( '/stdrepo/ANI/Asset/Rig/Project_rigs/creature/dog/slc_dog/forMCP/E9_rig_forKeySwitch_v001.mb', removeReference=True )
    ...
    #############################
    #############################    
    mocapTempLocators = ['tempWorldCon', 'temp_root', 'temp_L_foreLeg', 'temp_R_foreLeg', 'temp_L_wistRoll', 'temp_R_wistRoll','temp_L_hindLeg', 'temp_R_hindLeg','temp_L_ankleRoll', 'temp_R_ankleRoll','temp_L_elbow', 'temp_R_elbow', 'temp_L_knee', 'temp_R_knee']
    transCtrls = ['place_CON', 'C_IK_root_CON', 'L_IK_foreLeg_CON', 'R_IK_foreLeg_CON','L_wistRoll_CON','R_wistRoll_CON', 'L_IK_hindLeg_CON', 'R_IK_hindLeg_CON', 'L_ankleRoll_CON', 'R_ankleRoll_CON', 'L_IK_edbow_CON', 'R_IK_edbow_CON','L_IK_knee_CON', 'R_IK_knee_CON']
    mocapCtrls = ['C_IK_head_CON', 'C_IK_neckMiddle_CON', 'C_IK_neck1_CON', 'L_IK_clavicle_world_CON', 'L_IK_clavicle_CON', 'R_IK_clavicle_world_CON', 'R_IK_clavicle_CON', 'C_IK_upBody_CON', 'L_IK_scapula_CON', 'R_IK_scapula_CON', 'C_IK_spineMiddle_CON', 'C_IK_lowBody_CON', 'C_IK_hip_CON', 'L_IK_hip_CON', 'R_IK_hip_CON', 'tail1_CON', 'tail2_CON', 'tail3_CON', 'tail4_CON', 'L_IK_edbow_CON', 'R_IK_edbow_CON', 'L_IK_knee_CON', 'R_IK_knee_CON', 'C_IK_root_CON', 'L_wistRoll_CON', 'R_wistRoll_CON', 'L_ankleRoll_CON', 'R_ankleRoll_CON', 'R_IK_foreLeg_CON', 'L_IK_foreLeg_CON', 'L_IK_hindLeg_CON', 'R_IK_hindLeg_CON', 'move_CON', 'direction_CON', 'place_CON']
    ...
    keyPst = findMinMaxKey(mocapCtrls, nameSpace)
    cmds.currentTime(keyPst[0])
    setKeyAll(mocapCtrls, nameSpace)    
    cmds.currentTime(keyPst[0]-1)
    setKeyAll(mocapCtrls, nameSpace)    
    makeInitPose(mocapCtrls, initAtt_list, nameSpace)
    ...
    ########
    ########
    minCurrent = keyPst[0]-1
    maxCurrent = keyPst[1]

    for i in range(len(mocapTempLocators)):
    
        cmds.spaceLocator( p=(0, 0, 0), n=(mocapTempLocators[i]) )
        cmds.setAttr(mocapTempLocators[i]+".v",0)
        conPst = cmds.xform(nameSpace+":"+transCtrls[i], t=1, q=1, ws=1)
        cmds.xform(mocapTempLocators[i], t=(conPst[0],conPst[1],conPst[2]), ws=1)

    for i in range(len(mocapTempLocators)):
        cmds.parentConstraint(nameSpace+":"+transCtrls[i], mocapTempLocators[i], mo=1)

    cmds.select(cl=1)
    for loc in mocapTempLocators:
    ...
    for i in range(len(mocapTempLocators)):
        attExist = cmds.listAttr( nameSpace+":"+transCtrls[i], s=True, k=1, st='rotateX')
        if(attExist!=None):    
            cmds.parentConstraint(mocapTempLocators[i], nameSpace+":"+transCtrls[i], mo=1)

        else:
            cmds.pointConstraint(mocapTempLocators[i], nameSpace+":"+transCtrls[i], mo=1)


    cmds.delete(nameSpace+":place_CON", cn=1)
    cmds.cutKey(nameSpace+":place_CON", t=(minCurrent,maxCurrent))


    cmds.select(cl=1)
    for ctrl in transCtrls:
        cmds.select(nameSpace+":"+ctrl, add=1)
    ...
    for ctrl in ctrlList:
        if(cmds.objExists(nameSpace + ":" + ctrl)):
            cmds.cutKey(nameSpace + ":" + ctrl, clear=1, t=(minCurrent,minCurrent))
...
def abs_local(*argv):
    ...
    targetChar = Retarget()
    initialSetup(targetChar)


    keyPst = findMinMaxKey(transCtrls, nameSpace)
    cmds.currentTime(keyPst[0])
    setKeyAll(transCtrls, nameSpace)    
    cmds.currentTime(keyPst[0]-1)
    setKeyAll(transCtrls, nameSpace)    
    cmds.filterCurve( f='euler')     
    ...
    cmds.parent('temp_root', 'temp_L_foreLeg', 'temp_R_foreLeg', 'temp_L_wistRoll', 'temp_R_wistRoll','temp_L_hindLeg', 'temp_R_hindLeg','temp_L_ankleRoll', 'temp_R_ankleRoll','temp_L_elbow', 'temp_R_elbow', 'temp_L_knee', 'temp_R_knee','tempWorldCon')

    cmds.parentConstraint(nameSpace+':C_IK_root_CON','tempWorldCon', mo=1)
    cmds.parentConstraint(nameSpace+':C_IK_root_CON','temp_root', mo=1)
    cmds.parentConstraint(nameSpace+':L_IK_foreLeg_CON','temp_L_foreLeg', mo=1)
    cmds.parentConstraint(nameSpace+':R_IK_foreLeg_CON','temp_R_foreLeg', mo=1)
    cmds.parentConstraint(nameSpace+':L_wistRoll_CON','temp_L_wistRoll', mo=1)
    cmds.parentConstraint(nameSpace+':R_wistRoll_CON','temp_R_wistRoll', mo=1)
    cmds.parentConstraint(nameSpace+':L_IK_hindLeg_CON','temp_L_hindLeg', mo=1)
    cmds.parentConstraint(nameSpace+':R_IK_hindLeg_CON','temp_R_hindLeg', mo=1)
    cmds.parentConstraint(nameSpace+':L_ankleRoll_CON','temp_L_ankleRoll', mo=1)
    cmds.parentConstraint(nameSpace+':R_ankleRoll_CON','temp_R_ankleRoll', mo=1)
    cmds.parentConstraint(nameSpace+':L_IK_edbow_CON','temp_L_elbow', mo=1)
    cmds.parentConstraint(nameSpace+':R_IK_edbow_CON','temp_R_elbow', mo=1)
    cmds.parentConstraint(nameSpace+':L_IK_knee_CON','temp_L_knee', mo=1)
    cmds.parentConstraint(nameSpace+':R_IK_knee_CON','temp_R_knee', mo=1)
...
def bake_world(*argv):
    ...
    cmds.currentTime(keyPst[0]-1)
    setKeyAll(transCtrls, nameSpace)    
    makeInitPose(transCtrls, initAtt_list, nameSpace)
    cmds.filterCurve( f='euler')     
...
def bake_local(*argv):
    ...
    cmds.pointConstraint(nameSpace+':C_IK_root_CON','tempWorldCon', mo=1, skip="y")
    cmds.parentConstraint(nameSpace+':C_IK_root_CON','temp_root', mo=1)
    cmds.parentConstraint(nameSpace+':L_IK_foreLeg_CON','temp_L_foreLeg', mo=1)
    cmds.parentConstraint(nameSpace+':R_IK_foreLeg_CON','temp_R_foreLeg', mo=1)
    cmds.parentConstraint(nameSpace+':L_wistRoll_CON','temp_L_wistRoll', mo=1)
...
##주어진 컨트롤러에 전체키
def setKeyAll(controlers, nameSpace):
    cmds.select(cl=1)
    for controler in controlers:
        if(cmds.objExists(nameSpace + ":" + controler)):
            cmds.select(nameSpace + ":" + controler, add=1)
...
##주어진 컨트롤러들의 초기화 포즈셋팅
def makeInitPose(controlers, attributes,nameSpace):
    #nameSpace = self.getNamespace()
    attList = attributes.keys()
...
def makeUI():
    ...
    windowUI = cmds.window('animRetargeter', t="anim retargeter", s=1, widthHeight=(200,55))
    cmds.window(windowUI, edit=1, rtf=1, widthHeight=(200,55))

    cmds.columnLayout(columnWidth = 500,adjustableColumn=False)
    ...
    cmds.button("mocapRetarget_Button", w=300, l='Converting motion capture', command=retarget_bake_EXE)
    cmds.separator()
    cmds.button("Bake_Local_Button", w=300, l='Bake controller(Local) ', command=bake_local)
    cmds.button("Bake_World_Button", w=300, l='Bake controller(World) ', command=bake_world)

    cmds.setParent('..')
    cmds.showWindow(windowUI)
'''

def bipedRetarget_EXE(*argv):

    BipedRetarget_edit.retargetEXE()
...
def makeUI():
    ...
    windowUI = cmds.window('humanRetargetWin', t="Retargeter (22.7)", s=0, widthHeight=(454,250))
    cmds.window(windowUI, edit=1, rtf=1, widthHeight=(454,267))

    cmds.columnLayout("frameLayout_column", w=465, h=250, ebg=1, cat=("left",10), adjustableColumn=False, io=1)
    cmds.separator(h=10)
    cmds.frameLayout("h_RetargetFrame",l="Biped",w=445, h=125, bv=1, bgs=1)
    cmds.rowColumnLayout("h_RetargetColumn",w=445,h=350,numberOfRows=1, rowHeight=[(1,100)], cs=(1,35))
    cmds.button("hik_RetargetButton", w=220, l='HIK', command=bipedRetarget_EXE)
    cmds.checkBox("IK_Blend", label='Matching foot position', v=1)
    ...
    #cmds.checkBoxGrp("retargetCheckBox", cal=[1,"center"], rat=[1,"both",10], numberOfCheckBoxes=1, label='HIK retargeting', v1=1, en=0) #retargetCheckBox)
    #cmds.setParent('..') 


    cmds.rowColumnLayout("CRD_human_column",w=445,h=80, numberOfRows=1, rowHeight=[(30,46)], cs=(1,1))
    cmds.button("quad_retarget_button", w=110, h=75, l='retargetEXE', en=1, command=retarget_EXE)
    cmds.button("Bake_Local_Button", w=110, l='Bake local', en=1, command=bake_local)
    cmds.button("Bake_World_Button", w=109, l='Bake world', en=1, command=bake_world)
    cmds.button("Fix_orient_axis_Button", w=109, l='', en=0, command='')
    cmds.setParent('..') 
    cmds.setParent('..') 
