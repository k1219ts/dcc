...
class UI(pm.uitypes.Window):
    ...
    def __init__(self):
        ...
        with form:
            ...
            pm.menuItem('Import Set', c=pm.Callback(self.set_import))
            pm.menuItem('Export Set', c=pm.Callback(self.set_export))
            pm.menuItem(divider=1)
    ...
    def set_import(self):
        setFile = pm.fileDialog2(fileFilter="*.mb", fm=1, dialogStyle=2, okCaption="open")
        if setFile:
            pm.importFile(setFile, mergeNamespacesOnClash=False, rpr='')
        self.set_load(1)

    def set_export(self):
        tabSets = Set.findSets()
        for i in tabSets:
            if Set.getName(i) == self.set_getActive().tabName:
                setNode = self.set_getActive().set
                exportFile = pm.fileDialog2(fileFilter="*.mb", fm=0, dialogStyle=2, okCaption="Export")
                if exportFile:
                    pm.select(setNode, r=True, ne=True)
                    pm.exportSelected(exportFile, typ="mayaBinary", f=True)
