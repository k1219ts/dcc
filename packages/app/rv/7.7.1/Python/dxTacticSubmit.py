...
class dxTacticSubmit(rvtypes.MinorMode):
    ...
    def chkMedia(self):
        ...
        if not media:
            ...
            if not 'tactic' in info['file']:
                self.ui.info_label.clear()
                self.ui.comment_textEdit.clear()
                self.MessagePopup(u'현재 열려있는 미디어가 tactic snapshot이 아닙니다.')
                return False
            else:
                return True
    ...
    def tacticSubmit(self):
        ...
        if not 0 < len(description):
            self.MessagePopup(u'입력 된 코멘트가 없습니다.')
            return
        else:
            description = '(%s)\n' % fileName + description

        result = self.MessagePopupOkCancel(u'업로드 하시겠습니까?')
        if result == QtWidgets.QMessageBox.Ok:
            ...
            try:
                ...
                if setting.contains('login'):
                    ...
                    self.MessagePopup(u'로그인 정보 오류입니다.\nVeloz에서 save login을 하셔야 합니다.')
                    return
            except:
                self.MessagePopup(u'로그인 정보 오류입니다.\nVeloz에서 save login을 하셔야 합니다.')
                return
            ...
            try:
                ...
                tactic.abort()
                self.MessagePopup(u'업로드 에러!!')
            else:
                tactic.finish()
                self.MessagePopup(u'성공적으로 업로드 되었습니다!!')
                self.ui.comment_textEdit.clear()
