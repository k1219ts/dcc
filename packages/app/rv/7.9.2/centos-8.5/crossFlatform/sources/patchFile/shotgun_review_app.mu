...
global Mode localMode = nil;

global regex PAINT_PEN_RE   = "^.*\\.pen:.*";
global regex PAINT_TEXT_RE  = "^.*\\.text:.*";
global regex PAINT_FRAME_RE = "^.*\\.frame:.*";
...
class: ReviewAppMinorMode : MinorMode
{
    ...
    // Convert from an index into the widgets array to a Qt::Orientation
    // Note: This helper is to be used with QMainWindow::resizeDock()
    method: indexToOrientation (int; int index)
    {
        if ( index == 0 || index == 1 ) 
    ...
    {
        deb("INFO: QWebEngineView::loadFinished=%s\n" % (ok));
        _progressBars[index].reset();
        if ( _floating[index] == 1 )
        {
    ...
    {
        deb("INFO: QWebEngineView::loadProgress=%d\n" % (value));
    ...
    method: initView (void; int index, string url, int startSize=0)
    {
        ...
        connect(_webViews[index], QWebEngineView.loadProgress, progress(index,));
        connect(_webViews[index], QWebEngineView.loadFinished, progressReset(index,));
        ...
        javascriptMuExport(_webViews[index].page());
        ...
        int i = index;
        let formerWidth = _sizes[2 * i];
        let formerHeight = _sizes[2 * i + 1];
        if ( formerWidth != -1 && formerHeight != -1 )
        {
            // If the pane is *not* docked (i.e. floating) then we set both the
            // height and width from the previous session
            if ( _floating[i] == 1 )
            {
                _baseWidgets[i].setMaximumSize( QSize(formerWidth, formerHeight) );
                _baseWidgets[i].setMinimumSize( QSize(formerWidth, formerHeight) );
            }
            ...
            else
            {
                ...
                let orientation = indexToOrientation(i);
                let dockSize = (if (orientation == Qt.Horizontal) then formerWidth else formerHeight);
                mainWin.resizeDock( _dockWidgets[i], dockSize, orientation );
            }            
        }
        else if (i == 0 || i == 1)
        {
            ...
            else
            {
                let mainWin = mainWindowWidget();
                let orientation = indexToOrientation(i);
                mainWin.resizeDock( _dockWidgets[i], startSize, orientation );
            }
        }
        else if (i == 2 || i == 3)
        {
    ...
    //--------------------------------------------------------------------------
    // Callback to execute once get_shotgun_info() has completed
    //
    method: _validate_shotgun_version_callback((void; string);)
    {
        \: (void; string shotgun_info )
        {
            // Minimum required Shotgun version that supports RV Qt5 integration is 8.6.0.
            let min_shotgun_version = "8.6.0";

            // Compare the current Shotgun version against the minimum required one
            let parts = regex.smatch("\"version\":\[([0-9][0-9]*),([0-9][0-9]*),([0-9][0-9]*)", shotgun_info);
            if (parts neq nil)
            {
                if ( parts.size() == 4 )
                {
                    let cur_shotgun_version = "%s.%s.%s" % (parts[1], parts[2], parts[3]);
                    let cur_parts = cur_shotgun_version.split(".");
                    let min_parts = min_shotgun_version.split(".");
                    for_index (i; cur_parts)
                    {
                        if (int(cur_parts[i]) > int(min_parts[i])) break;
                        else
                        if (int(cur_parts[i]) < int(min_parts[i]))
                        {
                            let msg = "Your Shotgun site is running a version that is too old to support this version of RV. Your current version of Shotgun is %s, you must be running %s or newer.\n\n" % (cur_shotgun_version, min_shotgun_version);
                            msg += "Contact your Shotgun Admin or support@shotgunsoftware.com to update your site to a compatible version.";
                            alertPanel(false,
                                       ErrorAlert,
                                       "Unsupported Shotgun version",
                                       msg,
                                       "Ok", nil, nil);
                        }     
                    }
    ...
    //
    //  Validate the shotgun version to make sure it is compatible with the review app
    //
    method: _validate_shotgun_version (void;)
    {
      deb( "_validate_shotgun_version()" );
      shotgun.theMode().get_shotgun_info( _validate_shotgun_version_callback() );
    }
    ...
    method: _openWebViewAtIndexToUrl(void; int index, string url)
    {
        ...
        if ( currentUrl.isEmpty() || currentUrl != newUrl ) {
            ...
            if ( _sessionUrl != "") {              
                deb("INFO: _openWebViewAtIndexToUrl - Loading Cookies\n");

                let webEnginePage = _webViews[index].page();
                let webEngineProfile = webEnginePage.profile();
                let webEngineCookieStore = webEngineProfile.cookieStore();
                
                // Set the http user agent so that RV can be recognized by Shotgun
                // For reference: function sg_is_in_rv() in the shotgun repo
                // /git/shotgun/public/javascripts/application.js
                let httpUserAgent = webEngineProfile.httpUserAgent();
                if ( !regex.match("( RV )", httpUserAgent) ) {
                    let rvVersionString = system.getenv("TWK_APP_VERSION");
                    let reviewAppVersion = packageVersion();
                    string reviewAppVersionString = "%d.%d" % reviewAppVersion;
                    httpUserAgent = httpUserAgent + " RV/" + rvVersionString + " ReviewApp/" + reviewAppVersionString + " ( RV )";
                    deb("INFO: httpUserAgent=%s\n" % (httpUserAgent));
                    webEngineProfile.setHttpUserAgent(httpUserAgent);
                }
                ...
                // element is the name of the cookie, the second is its value and
                // the third is its expiration time as a unix timestamp
                let SettingsValue.StringArray cookies = readSetting("ReviewApp", _sessionUrl, SettingsValue.StringArray(string[]{}) );

                // Try to reach the SLModule registry to get the last session cookie
                (string, string, string, string, string, string, string) slSessionParam = ("", "", "", "", "", "", "");
                bool isRVDefaultSessionActive = isUrlInLastSessionFromSLUtils(_sessionUrl);

                QUrl sessionUrl = QUrl(_sessionUrl);
                ...
                if (isRVDefaultSessionActive) {
                    ...
                    deb("INFO: Adding session cookie\n");                                                                   
                    let x = QDateTime.fromTime_t((QDateTime.currentMSecsSinceEpoch() / 1000) + 172800); // Assume RV timestamp is unavailable by default, give two day
                    ...
                    deb("INFO: _sessionUrl=%s, _session_id=%s\n" % (_sessionUrl, slSessionParam._2) );

                    // Add the session id cookie if any
                    if (slSessionParam._2 != "") {
                        deb("INFO: Adding session id cookie\n");
                        ...
                        webEngineCookieStore.setCookie(c, sessionUrl);                         
                        deb("INFO: Patching cookie %s=%s\n" % ("_session_id", slSessionParam._2));
                    }            

                    // Add the Cross-Site Request Forgery (CSRF) token cookie if any
                    if (slSessionParam._4 != "" && slSessionParam._5 != "") {
                        deb("INFO: Adding Cross-Site Request Forgery (CSRF) token cookie\n");
                        ...
                        webEngineCookieStore.setCookie(c, sessionUrl);                         
                        deb("INFO: Patching cookie %s=%s\n" % (slSessionParam._4, slSessionParam._5));
                    }            
                }

                // Delete the SGES5 cookie if any 
                // Note that the current version of RV supports the latest javascript.
                let n = QByteArray().append("SGES5");
                let v = QByteArray().append("false");
                let c = QNetworkCookie(n, v);
                webEngineCookieStore.deleteCookie(c, sessionUrl);                         
            }

            // Validate the shotgun version to make sure it is compatible with the review app
            _validate_shotgun_version();
    ...
    method: compareFromVersionIds(void; int[] ids)
    {
        ...
        try
        {
        for_each (n; string[] {"defaultStack", "reviewAppStackGroup"})
        {
            for_each (p; properties("%s_paint" % n)) 
            {
                if (regex.match("^%s_paint\\.pen:.*" % n, p) ||
                    regex.match("^%s_paint\\.text:.*" % n, p) ||
                    regex.match("^%s_paint\\.frame:.*" % n, p))
                {
                    deleteProperty(p);
                }
            }
    ...
    method: createSourcesForInfos(void; StringMap[] infos, (void; string[]) afterFunc)
    {
        ...
        for_each(inf; infos) {
            ...
            let edlMediaType = shotgun.theMode().mediaTypeFallback (shotgun.theMode()._prefs.loadMedia, inf, shotgun.theMode()._prefs.autoStream);
    ...
    {
        let node = "reviewAppSequenceGroup";
        if ( !nodeExists(node) ) {
    ...
    method: openInFileSystem(void; StringMap[] infos) {
        ...
        string mediaType = shotgun.theMode().mediaTypeFallback(mediaPref, inf, shotgun.theMode()._prefs.autoStream);
...
""" % (f, sourceFilePath, f, f));
    ...
    method: getAnnotatedFramesForCurrentSource(int[];) {
        ...
        deb ("getAnnotatedFramesForCurrentSource returns %s\n" % framesToExport);
    ...
    method: uploadAnnotatedFramesToNote(ExternalProcess; int noteId) {
        ...
        for_index(i; framesToExport) {
            ...
            if ( i == -1 ) {
                ...
                // The offset value calculated here will be added to
                // any global frame between rangeStart and rangeEnd to produce 
                // local frame numbers in the sg_frameburn overlay script.
                int offset = localFrame - f;
        ...
        string filepat = "%s/annot_version_%s_v2.#.jpg" % (dirName, versionId);
    ...
    method: annotationExportCompleted(void; int noteId, string dirName) {
        ...
        for_each(fileInfo; contents) {
            ...
            deb("annotationExportCompleted() oldfilename '%s'\n" % fileName);
            ...
            if ( parts neq nil ) {
                ...
                int frame = sourceFrame(int(parts.back()));

                // rename the file to remove trailing zeros in the annotation
                newFileName = regex.replace("[0-9]+\\.jpg$", fileName, "%d.jpg" % frame);
            ...
            deb("annotationExportCompleted() new file path '%s'\n" % fileInfo.absoluteFilePath());
            filePaths.push_back(fileInfo.absoluteFilePath());
            displayNames.push_back(newFileName);
        }
    ...
    method: propertyChangedTimeout(void; ) {
        ...
        // If RV was opened with start playback (-play) option, and media is not 
        // currently playing, then start playback.
        playCurrent();
    }
    ...
    method: afterGraphViewChange(void; Event event) {
        ...
        if (event neq nil && (
                event.contents() == "reviewAppStackGroup" ||
                event.contents() == "reviewAppLayoutGroup"))
        //
        //  We're switching to one of the "Compare" nodes, which we consider
        //  transitory "scratch" nodes, so ensure there is no paint left over
        //  from a previous visit.
        {        
            for_each (p; properties(event.contents() + "_paint")) 
            {
                if (PAINT_PEN_RE.match(p)  ||
                    PAINT_TEXT_RE.match(p) ||
                    PAINT_FRAME_RE.match(p))
                {
                    deleteProperty(p);
                }
            }
    ...
    method: _updateIfStateChanged(void; bool force) {
        ...
        if ( enableUpdates ) {
            ...
            // We might have missed a propertyChanged event while the frame
            // events were unbound. If this is the case then trigger it.
            if (_sessionIsLoading) {
    ...
    method: _generateUrl(QUrl; QUrl desiredUrl)
    {
        ...
        if ( serverName neq nil ) {
            ...
            QUrlQuery fallbackUrlQuery = QUrlQuery();
            fallbackUrlQuery.addQueryItem("return_to", string(returnToEncoded));
            fallbackUrl.setQuery(fallbackUrlQuery);
            ...
            urlQuery.addQueryItem("url", string(encodedDesiredUrl));
            urlQuery.addQueryItem("fallback", string(encodedFallbackUrl));
    ...
    method: internalLaunchTimeline(void; [(string, string)] urlArgs = nil)
    {
        ...
        for_each(urlArg; urlArgs) {
            ...
            urlQuery.addQueryItem(urlArg._0, urlArg._1);
        }
        if (!urlQuery.isEmpty())
        {
            desiredUrl.setQuery(urlQuery);
        }
    ...
    method: openEncodedUrl(void; string url)
    {
        ...
        // Tell QT that the URL is already correctly encoded, don't try to "fix" it.
        openUrlFromUrl(QUrl.fromEncoded(qba, QUrl.StrictMode));
    }
    ...
    method: _buildPrefsMenu(Menu; shotgun_mode.ShotgunMinorMode shotgunMode )
    {
        ...
        Menu m2 = Menu {
            ...
            {"    Enable Streaming", shotgunMode.toggleStreaming, nil, shotgunMode.autoStreaming},
        };
    ...
    method: deactivate(void;)
    {
        ...
        for_each (w; _webViews)    if (w neq nil) w.page().setHtml("", QUrl());
    ...
    method: ReviewAppMinorMode (ReviewAppMinorMode;)
    {
        ...
        _webViews     = QWebEngineView[](); _webViews.resize(4);
        ...
        // We've implemented our own QWebEngineCookieStore so that we can save
        ...
        let s = QWebEngineSettings.globalSettings();
        s.setAttribute(QWebEngineSettings.LocalContentCanAccessRemoteUrls, true);
        s.setAttribute(QWebEngineSettings.PluginsEnabled, false);
        //s.setAttribute(QWebEngineSettings.DeveloperExtrasEnabled, SGR_DEBUG);
        ...
        _mystate = "unknown";
        let SettingsValue.Int freq = readSetting("ReviewApp", "updateFrequency", SettingsValue.Int(-2));
        _updateFrequency = freq;
...
\: localModeReady(bool; )
{
    return (localMode neq nil);
}

\: createLocalMode(void; )
{
    State state = data();
    createMode();
    state.minorModes.push_back(localMode);
    localMode.activate();
}

\: theMode (ReviewAppMinorMode;)
{
    ReviewAppMinorMode m = localMode;

    if (m eq nil) 
    {
        print ("--------------------------------- review_app localMode is nil!\n");
        m = rvui.minorModeFromName("Screening Room");
    }
