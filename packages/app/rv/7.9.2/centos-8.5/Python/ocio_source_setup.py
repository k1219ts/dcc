...
def ocioActiveOnAllEvent(config, nodeType, active=True):
    def useSourceOCIO(source):
        ...
        try:
            ...
            attrDict["default_setting"] = OCIO_DEFAULTS[nodeType]
        except:
            attrDict = {}
        ...
        try:
            ...
            pipelineList = ocio_node_from_media(config, srcPipeline, DEFAULT_PIPE[pipeSlot], media, attrDict)
    ...
    def F (event):
        for frame in range(commands.frameStart(), commands.frameEnd() + 1):
            evalInfo = commands.metaEvaluateClosestByType(frame, "RVFileSource", None)
            if len(evalInfo) == 0:
                evalInfo = commands.metaEvaluateClosestByType(frame, "RVImageSource", None)
            if len(evalInfo) == 0:
                return
            source = evalInfo[0]["node"]

            if active:
                useSourceOCIO(source)
            else:
                disableSourceOCIO(source)
...
class OCIOSourceSetupMode(rvtypes.MinorMode):
    ...
    def buildOCIOMenu(self):
        ...
        csaList = [
            ("All Active", ocioActiveOnAllEvent(self.config, "OCIOFile"), '', None),
            ("All Deactive", ocioActiveOnAllEvent(self.config, "OCIOFile", False), '', None),
            ("_", None)
        ]
