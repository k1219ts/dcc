...
class VersioningMode(rvtypes.MinorMode):
    ...
    def saveMovList(self, event):
        if '.rv' not in commands.sessionFileName():
            self.MessagePopup(u'지금 열려 있는 파일은 rv session File이 아닙니다.')
            return
        ...
        fj.close()
        self.MessagePopup(u'샷 네임이 저장되었습니다.\n%s' % filepath)
    ...
    def reloadSnapShot(self, event):
        ...
        result = self.MessagePopupOkCancel(u'최신 snapshot으로 update 하시겠습니까?')
        ...
        if changedShot:
            self.MessagePopup(u'최신 snapshot으로 update 되었습니다.\n'+changedShot)
        else:
            self.MessagePopup(u'모두 최신 snapshot입니다.')
    ...
    def chkMedia(self):
        ...
        if not media:
            self.MessagePopup(u'열려있는 미디어가 없습니다.')
            return False
        else:
            ...
            if not 'show' in info['file']:
                self.MessagePopup(u'현재 미디어가 tactic snapshot이 아닙니다.')
                return False
            elif '/asset/' in info['file']:
                self.MessagePopup(u'현재 미디어가 shot mov가 아닙니다.')
                return False
            else:
                return True
    ...
    def openDexterGuide(self, event):
        ...
        if chkplatform != 'Darwin':
            ...
            self.MessagePopup(u'해당 기능은 centOS에서만 지원합니다.')
