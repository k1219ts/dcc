...
def UI(*args):
    ...
    #getting rid of '\n'
    mariHosts = map(lambda s: s.strip(), items)
...
def sendObjToMari(*args):
    ...
    try:
        ...
        print "ERROR - Please Make Sure Port 6100 is Open in Mari"
        cmds.inViewMessage(amg='<hl>ERROR</hl> - Please Make Sure Port 6100 is Open in Mari', pos='botCenter',
                           fade=True, fadeOutTime=500)
        return

    outputPath = assetsPath + "/" + projectName + "/" + "%s.abc" % projectName
    expMesh = dxExportMesh.ExportMesh(outputPath, cmds.ls(sl=True, l=True))

    uvClass = dxExportMesh.UVLayOut()
    txlayout = uvClass.layoutInfo()
    txindexs = []
    ...
    if scenePath.startswith('/netapp/dexter/show'):
        scenePath = outputPath.replace('/netapp/dexter/show', '/show')

    splitPath = scenePath.split("/")
    show = splitPath[splitPath.index("show") + 1]
    assetName = projectName

    # Initial the creation project in Mari.
    if sendMode == 1:
        mari.send('dxMtm.setupProject(%s, %s, %s)' % (show, assetName, outputPath))
        mari.close()
    elif sendMode == 2:
        mari.send('dxMari.addObject("%s")\x04' % outputPath)
    elif sendMode == 3:
        mari.send('dxMari.addVersion("%s")\x04' % outputPath)
