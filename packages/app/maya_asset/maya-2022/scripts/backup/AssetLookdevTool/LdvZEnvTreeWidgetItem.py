...
class LdvZEnvTreeWidgetItem(QtWidgets.QTreeWidgetItem):
    def __init__(self, parent, filePath, attrIndex = -1):
        ...
        decodingRule = self.rulebook.decode(filePath, "env_asset_path")
        ...
        self.envAssetPath = "{0}/model".format(str(self.rulebook.product["env_asset_path"]))
        ...
        self.assetLabel.setText(self.sourceName)
    ...
    def loadAlembic(self, ver):
        ...
        if ver == -1:
            ...
            cmds.setAttr('%s.ldvZEnvAbcName[%d]' % (self.ldvNode, self.attrIndex), self.alembicFileName, type='string')
        else:
            self.alembicFileName = cmds.getAttr('%s.ldvZEnvAbcName[%d]' % (self.ldvNode, self.attrIndex))
    ...
    def changeAlembicImport(self):
        ...
        cmds.setAttr('%s.ldvZEnvAbcVer[%d]' % (self.ldvNode, self.attrIndex),
    ...
    def loadTexture(self, ver):
        ...
        if not (ver == -1):
            ...
            cmds.setAttr('%s.ldvZEnvTexVer[%d]' % (self.ldvNode, self.attrIndex), self.textureVersionComboBox.currentText(),
                         type='string')
    ...
    def loadShader(self, ver):
        ...
        if not (ver == -1):
            ...
            cmds.setAttr('%s.ldvZEnvShaderVer[%d]' % (self.ldvNode, self.attrIndex), self.shaderVersionComboBox.currentText(),
                         type='string')
    ...
    def deleteItemInfo(self):
        ...
        size = cmds.getAttr('%s.ldvZEnvPath' % self.ldvNode, size=True)
        for index in range(self.attrIndex, size - 1):
            cmds.setAttr('%s.ldvZEnvPath[%d]' % (self.ldvNode, index),
                         cmds.getAttr('%s.ldvZEnvPath[%d]' % (self.ldvNode, index + 1)), type='string')

            cmds.setAttr('%s.ldvZEnvAbcVer[%d]' % (self.ldvNode, index),
                         cmds.getAttr('%s.ldvZEnvAbcVer[%d]' % (self.ldvNode, index + 1)), type='string')

            cmds.setAttr('%s.ldvZEnvTexVer[%d]' % (self.ldvNode, index),
                         cmds.getAttr('%s.ldvZEnvTexVer[%d]' % (self.ldvNode, index + 1)), type='string')

            cmds.setAttr('%s.ldvZEnvShaderVer[%d]' % (self.ldvNode, index),
                         cmds.getAttr('%s.ldvZEnvShaderVer[%d]' % (self.ldvNode, index + 1)), type='string')

            cmds.setAttr('%s.ldvZEnvAbcName[%d]' % (self.ldvNode, index),
                         cmds.getAttr('%s.ldvZEnvAbcName[%d]' % (self.ldvNode, index + 1)), type='string')

        print (self.ldvNode, size - 1)
        mel.eval('AEremoveMultiElement %s.ldvZEnvPath[%d]' % (self.ldvNode, size - 1))
        mel.eval('AEremoveMultiElement %s.ldvZEnvAbcVer[%d]' % (self.ldvNode, size - 1))
        mel.eval('AEremoveMultiElement %s.ldvZEnvTexVer[%d]' % (self.ldvNode, size - 1))
        mel.eval('AEremoveMultiElement %s.ldvZEnvShaderVer[%d]' % (self.ldvNode, size - 1))
        mel.eval('AEremoveMultiElement %s.ldvZEnvAbcName[%d]' % (self.ldvNode, size - 1))
