...
if __name__ == '__main__':
    ...
    optparse.add_option(
        '-f', dest='filter', type='string', default='default.filter.json',
        help='Default: default.filter.json , Example: default.filter.json, volume.filter.json, sigmaAlphaOnly.filter.json'
    )
    optparse.add_option(
        '-s', '--strength', dest='strength', type='float', default=0.5,
        help='Default: 0.5 , Type: float , Example: 0.2'
    )
    optparse.add_option(
        '-c', '--crossframe', dest='crossframe', action='store_true', default=False,
        help='Default: False , Option: store_true'
    )
    optparse.add_option(
        '--frame', dest='frame', type='int', default=0,
    ...
    (opts, args) = optparse.parse_args(sys.argv)
    opts.varianceimage = ''

    inputfile   = sys.argv[-1]

    if len(args) > 2:
        opts.varianceimage = args[1]
    else:
        varfile = GetVarianceFile(inputfile)
        if varfile:
            opts.varianceimage = varfile

    if inputfile.split('.')[-1] != 'exr':
        print '[ERROR] : Wrong arguments!'
        os._exit(0)

    cc = Main(opts, inputfile)
