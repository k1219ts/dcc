...
class DxFoam : public RixPattern
{
    public:
        ...
        virtual int ComputeOutputParams(
            ...
            RtInt *noutputs, RixPattern::OutputSpec **outputs,
...
)
{
    ...
    *noutputs = 1;
    ...
    for (int i=0; i<sctx->numPts; i++)
    {
        ...
        // common parameter
        SeExpr::VoronoiPointData pdata;
        pdata.points[0]= 1.0;// Q[i].x?
        pdata.points[1]= 1.0;// Q[i].y?
        pdata.points[2]= 1.0;// Q[i].z?
        SeVec3d vtype, vjitter, vfbmScale, vfbmOctaves, vfbmLacunarity, vfbmGain;
        vtype[0]= 4;
        ...
        p1[2]= 1;
        SeVec3d args1[7]= {p1, vtype, vjitter, vfbmScale, vfbmOctaves, vfbmLacunarity, vfbmGain};
        double vor1;
        vor1= 1 - SeExpr::voronoiFn( pdata, 7, &args1[0] )[0];
        ...
        p2[2]= 1;
        SeVec3d args2[7]= {p2, vtype, vjitter, vfbmScale, vfbmOctaves, vfbmLacunarity, vfbmGain};
        double vor2;
        vor2= 1 - SeExpr::voronoiFn( pdata, 7, &args2[0] )[0];
        ...
        p3[2]= 1;
        SeVec3d args3[7]= {p3, vtype, vjitter, vfbmScale, vfbmOctaves, vfbmLacunarity, vfbmGain};
        double vor3;
        vor3= 1 - SeExpr::voronoiFn( pdata, 7, &args3[0] )[0];
        ...
        p4[2]= 1;
        SeVec3d args4[7]= {p4, vtype, vjitter, vfbmScale, vfbmOctaves, vfbmLacunarity, vfbmGain};
        double vor4;
        vor4= 1 - SeExpr::voronoiFn( pdata, 7, &args4[0] )[0];
...
//        fbm1= SeExpr::fbm( 4, &args5[0] );
        ...
        if(softness == 0)
        {
            vor1= SeExpr::gamma(vor1, seGamma[i]*0.05);
            vor2= SeExpr::gamma(vor2, seGamma[i]*0.05);
            vor3= SeExpr::gamma(vor3, seGamma[i]*0.05);
            vor4= SeExpr::gamma(vor4, seGamma[i]*0.05);
            resultF[i]= vor1 + vor2 + vor3 + vor4;
            resultF[i]= SeExpr::gaussstep(resultF[i], 0, 1.5);
            resultF[i]= SeExpr::gamma(resultF[i], 0.5);
//            resultF[i]= resultF[i] * fbm1;
        }else{
            resultF[i]= vor1 + vor2 + vor3 + vor4;
            resultF[i]= SeExpr::gaussstep(resultF[i], 0, 2.0);
            resultF[i]= SeExpr::gamma(resultF[i], seGamma[i]*0.5);
        }
    }
