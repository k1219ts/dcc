...
//
// LASTRELEASE
//  -2017.09.01 : primitive index for OpenEXRId
//                - add object_id (auto)
  ...
  , curveRoot(1.0)
{
    ...
    for ( size_t i = 0; i < tokens.size(); ++i )
    {
        ...
        else if ( token == "-subdiv" )
        {
            ...
            if( i < tokens.size() )
            {
                subdiv = atoi( tokens[i].c_str() );
                subdiv_defined = true;
            }
        }
        else if ( token == "-subframe" )
        {
            ++i;
            if( i < tokens.size() )
            {
                subframe = atoi( tokens[i].c_str() );
                subframe_defined = true;
            }
        }
        else if ( token == "-oid" )
        {
        ...
        else if ( token == "-txvar" )
        {
            ...
            if( i < tokens.size() )
            {
                txvar = atoi( tokens[i].c_str() );
                txvar_defined = true;
            }
        }
        else if ( token == "-primvar" )
        {
            ++i;
            if( i < tokens.size() )
            {
                primvar = atoi( tokens[i].c_str() );
                primvar_defined = true;
            }
        }
        else if ( token == "-cycle" )
        {
            ...
            if( i < tokens.size() )
            {
                cycle = atoi( tokens[i].c_str() );
                cycle_defined = true;
            }
        }
	    // add_charles : $0 edited
	    else if ( token == "-atjson" )
	    {
	        ++i;
	        if( i < tokens.size() )
	        {
		        atJson = json_object_from_file(tokens[i].c_str());

		        if ( atJson == NULL )
		        {
		            std::cout << "[AlembicRiProc-Warning] : "
                                      << "Cannot read the json file for attributes.("
			              << tokens[i].c_str() << ")" << std::endl;
		        }
		        else
		        {
		            json_object_object_get_ex(atJson, "Attributes", &atJson);
        ...
        // end
        else if ( token == "-curvetip" )
        {
            ++i;
            if( i < tokens.size() )
            {
                curveTip = atof( tokens[i].c_str() );
                curveTip_defined = true;
            }
        }
        else if ( token == "-curveroot" )
        {
            ++i;
            if( i < tokens.size() )
            {
                curveRoot = atof( tokens[i].c_str() );
                curveRoot_defined = true;
            }
        }
...
void ProcArgs::applyArgs(ProcArgs & args)
{
    ...
    if ( args.subdiv_defined )
    {
        subdiv = args.subdiv;
        subdiv_defined = true;
    }

    if ( args.subframe_defined )
    {
    ...
    if ( args.primvar_defined )
    {
        primvar = args.primvar;
        primvar_defined = true;
    }

    if ( args.txvar_defined )
    {
        txvar = args.txvar;
        txvar_defined = true;
    }

    if ( args.cycle_defined )
    {
        cycle = args.cycle;
        cycle_defined = true;
    }
    ...
    // charles edit
    if ( args.atJson_defined )
    {
	    atJson = args.atJson;
	    atJson_defined = true;
    }
    // end
    if ( args.curveTip_defined )
    {
        curveTip = args.curveTip;
        curveTip_defined = true;
    }
    if ( args.curveRoot_defined )
    {
        curveRoot = args.curveRoot;
        curveRoot_defined = true;
    }
