...
# changed by Taehyung Lee (lth1722@gamil.com)
...
# 2.0    taehyung.lee    pyside2, uic,  database, new publish path
...
def getPubVersion(show, seq, shot, data_type,plateType=None):
...
def checkImagePlanePath():
    ipList = mc.ls(
        type="imagePlane")  # result: ['perspShape2->imagePlaneShape1', 'imagePlaneShape2', ...]
    for i in ipList:
...
def renewImagePlane():
    ...
    for i in ipList:
        ...
        camShape = \
        mc.listConnections(i, type='camera', shapes=True, source=False)[0]
        ...
        if attrResult['useFrameExtension']['value']:
            mc.expression(n='frame_ext_expression',
...
def renewImagePlane_backup():
    ...
    for i in ipList:
        ...
        camShape = mc.listConnections(i, type="camera", shapes=True)[
            0]  # result: ['perspShape2']
...
def convertPolyImagePlane(renewIpList):
    ...
    for i in renewIpList:
        ...
        poly = mc.polyPlane(w=1.0, h=aspect, sx=1, sy=1, ax=(0, -1, 0), cuv=1,
                            ch=1)
        roo = mc.getAttr(camTransform + ".rotateOrder")
        # mc.setAttr(poly[0]+".rotateOrder", roo)
        noNamespaceCam = camTransform.split(":")[-1]
        newPlane = mc.rename(poly[0],
                             "layoutImagePlane_" + noNamespaceCam + "_temp")
        ...
        mc.setAttr(newPlane + ".sx",
                   (2 * (depth) * (math.tan(fov / 2.0))) / overscan)
        mc.setAttr(newPlane + ".sz",
                   (2 * (depth) * (math.tan(fov / 2.0))) / overscan)

        newPlane_new = mc.duplicate(name="layoutImagePlane_" + noNamespaceCam)[
            0]

        mc.parent(newPlane_new, world=True)

        Sconst = mc.scaleConstraint(newPlane, newPlane_new,
                                    maintainOffset=False)

        const = mc.parentConstraint(newPlane, newPlane_new,
                                    maintainOffset=False)
        ...
        mc.bakeResults(newPlane_new, t=(mc.playbackOptions(q=True, ast=True),
                                        mc.playbackOptions(q=True, aet=True)),
                       pok=True, sm=True,
                       at=["tx", "ty", "tz", "rx", "ry", "rz", "sx", "sy",
                           "sz"])
