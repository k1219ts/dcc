...
class CameraAssetAnim(object):
    def __init__(self, shot_info={}, scene_options={}, cam_options={}):
        ...
        print "shot", shot_info
        print "scene", scene_options
        print "cam", cam_options
    ...
    def publish(self):
        ...
        try:
            ...
            if self.shot_info["dept"] == "animation":
                self.anim_pub_path = os.path.join(self.shot_path, "ani", "pub", "scenes") # /show/prat/shot/SHS/SHS_0420/ani/pub/scenes/
                self.pub_filename = "%s_layout"%self.shot_info["shot"]   # result: ELX_0100_layout
            elif self.shot_info["dept"] == "matchmove":
                #self.anim_pub_path = os.path.join(self.shot_path, "layout", "pub", "scenes") # /show/prat/shot/SHS/SHS_0420/ani/pub/scenes/
                self.anim_pub_path = os.path.join(self.shot_path, "matchmove", "pub",
            ...
            for c in self.cam_options["cam_list"]:
                ...
                self.tempCamList.append(mc.rename(tmp_camera[0], "%s_layoutCamera%s"%(self.shot_info["shot"], self.cam_orientation)))
            ...
            self.pub_maya_scene_path = os.path.join(self.anim_pub_path, self.pub_filename+"_v%.2d"%(self.scene_ver)+".mb")
            try:
                mc.file(rename=self.pub_maya_scene_path)
                mc.file(save=True)
                print "Animation Scene Published! %s"%self.pub_maya_scene_path
                QtWidgets.QMessageBox.information(None, "Information", "Animation Scene Published!\n\n%s"%self.pub_maya_scene_path, QtWidgets.QMessageBox.Ok)
            except:
                QtWidgets.QMessageBox.critical(None, "Error", "Animation Scene Failed!\n\n%s"%self.pub_maya_scene_path, QtWidgets.QMessageBox.Ok)
            ...
            #self.camera_pub_path = os.path.join(self.shot_path, "camera", "pub", "scenes") # /show/prat/shot/SHS/SHS_0420/camera/pub/scenes/
            self.camera_pub_path = os.path.join(self.shot_path, "matchmove", "pub",
            ...
            #
            # self.cam_ver = getPubVersion(show=self.shot_info['show'],
            #                                seq=self.shot_info['seq'],
            #                                shot=self.shot_info['shot'],
            #                                data_type='camera')
            self.cam_ver = self.scene_ver
            ...
            self.pub_maya_cam_path = os.path.join(self.camera_pub_path, self.pub_filename+"_v%.2d"%(self.cam_ver+1)+".mb")
            try:
                mc.file(self.pub_maya_cam_path, op="v=0", typ="mayaBinary", preserveReferences=True, constructionHistory=True, channels=True, constraints=False, expressions=False, shader=False, exportSelected=True, exportAll=False, force=True)
                print "Maya Camera Published! %s"%self.pub_maya_cam_path
                self.dbRecord['files']['maya_pub_file'] = [self.pub_maya_cam_path]
                # QtGui.QMessageBox.information(None, "Information", "Maya Camera Published!\n\n%s"%self.pub_maya_cam_path, QtGui.QMessageBox.Ok)
            except:
                QtWidgets.QMessageBox.critical(None, "Error", "Exporting Maya Camera Failed!", QtWidgets.QMessageBox.Ok)
                print traceback.format_exc()
                return False

            if self.cam_options["fbx_cam"] == 2:
                make_dir(self.camera_pub_path)
                self.pub_fbx_cam_path = os.path.join(self.camera_pub_path, self.pub_filename+"_v%.2d"%(self.cam_ver+1)+".fbx")
                try:
                    ...
                    QtWidgets.QMessageBox.critical(None, "Error", "Exporting FBX Camera Failed!", QtWidgets.QMessageBox.Ok)
                    print traceback.format_exc()
                    return False

            if self.cam_options["abc_cam"] == 2:
                make_dir(self.camera_pub_path)
                self.pub_abc_cam_path = os.path.join(self.camera_pub_path, self.pub_filename+"_v%.2d"%(self.cam_ver+1)+".abc")
                try:
                    ...
                    abc_job += "-uvWrite -frameRange %s %s -file %s"%(self.scene_options["start_frame"], self.scene_options["end_frame"], self.pub_abc_cam_path)
                    print abc_job
                    mc.AbcExport(j=abc_job)
                    print "Alembic Camera Published! %s"%self.pub_abc_cam_path
                    self.dbRecord['files']['camera_path'] = [self.pub_abc_cam_path]

                except:
                    QtWidgets.QMessageBox.critical(None, "Error", "Exporting Alembic Camera Failed!",
            ...
            for sdb in self.sdb_list:
                try:
                    ids.append(mc.getAttr(sdb + ".objectId"))
                except:
                    pass

            taskDic['cameraIds'] = ids
            taskDic['plateType'] = 'layout'
            #
            self.dbRecord['tags'] = tag_parser.run(self.pub_abc_cam_path)
            self.dbRecord['task_publish'] = taskDic

            client = MongoClient(DB_IP)
            db = client[DB_NAME]
            coll = db[self.dbRecord['show']]
    ...
    def write_scene_json(self):
        ...
        sceneJSON["version"] = str(self.scene_ver)
    ...
    def write_cam_json(self):
        ...
        if len(self.sdb_list) > 0:
            for sdb in self.sdb_list:
                ...
                if os.path.isfile(jsonPath):
                    ...
                    elif jsdb.has_key('abcCam'):
                        mayaCamPath = os.path.split(jsdb["abcCam"])[1]
                        # export layout transform
                        self.write_layout_json(sdb)
                        mayaCamPath = mayaCamPath.replace(".abc", ".layout")

                    sdbCamList.append(mayaCamPath)
                else:
                    QtWidgets.QMessageBox.critical(None, "Error", "Reading Camera JSON Failed!",
                                                   QtWidgets.QMessageBox.Ok)
        else:
            print "no sdblist."
        ...
        try:
            ...
            QtWidgets.QMessageBox.critical(None, "Error", "Writting Camera JSON Failed!",
                                           QtWidgets.QMessageBox.Ok)
            print traceback.format_exc()
            return False
        finally:
    ...
    def write_layout_json(self, SDBNode):
        ...
        if os.path.isfile(mmv_json):
            ...
            QtWidgets.QMessageBox.critical(None, "Error", "Reading Matchmove JSON Failed!",
                                           QtWidgets.QMessageBox.Ok)

        if j.has_key("mayaCam"):
            layoutPath = j["mayaCam"].replace(".mb", ".layout")
        elif j.has_key("abcCam"):
            layoutPath = j["abcCam"].replace(".abc", ".layout")
