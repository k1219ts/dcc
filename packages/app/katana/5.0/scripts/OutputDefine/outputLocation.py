...
import dfk
import dxTactic.dxTacticCommon as dxTacticCommon
...
def locationSetup(node):
    ...
    if variableGroup:
        ...
        if 'shot' in variables and 'shot' in nodeVariants:
            ...
            if shotName.count('_') > 1:
                spltShotName = shotName.split('_')[-1]
                repSeqName = shotName.replace('_' + spltShotName, '')
                # Find Tactic seq
                seqCode = dxTacticCommon.getShowList()[name]['code']
                shotList = dxTacticCommon.getSeqList(seqCode)
                if repSeqName in shotList:
                    seqName = repSeqName
